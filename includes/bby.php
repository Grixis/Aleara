...
//count gained credits and experience and loststat
	...
	if ($attacker -> hp > 0 && $defender -> hp <= 0) {
		...
		$attacker -> gainexp = rand(20000,20000) * $defender -> level;
	  $attacker -> gaingold = rand(2000000,2000000) * $defender -> level;
		...
		loststat($attacker);
		$ggold = $attacker -> credits * 0;
		$ggold = (($ggold > 0)? $ggold: 0);
		$defender -> gaingold = (int) $ggold;
		$attacker -> gaingold = - (int) $ggold;
		$defender -> gainexp = rand(0,0) * $attacker -> level;
		if (rand (0,0) == 0 && $attacker -> maps > 0) {
			$defender -> maps++;
			$attacker -> maps--;
			$strMap = AND_MAP;
			}
