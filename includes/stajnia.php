...
if (isset ($_GET['action']) && $_GET['action'] == 'land') {
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'buy') {
        if (!$house -> fields['id']) {
            ...
            error ("Nauczyłes sie jeĽdzic na koniu. Teraz możesz udać się do <a href=stajnia.php?action=build>Stajni</a> aby zabaczyc konie na sprzedaż.");
        } else {
            if ($player -> credits < $cost1) {
                error ("Nie masz tylu sztuk złota przy sobie!");
            }
            $db -> Execute("UPDATE kon SET size=size+1 WHERE id=".$house -> fields['id']);
            $db -> Execute("UPDATE players SET credits=credits-".$cost1." WHERE id=".$player -> id);
            error ("Po solidnym treningu twój koń urusł o 1. <a href=stajnia.php>Wróć</a>");
        }
    }
}
...
if(isset ($_GET['action']) && $_GET['action'] == 'build') {
    ...
    if ($house -> fields['points'] == 0) {
        error ("Nie masz punktów treningowych aby trenować jakiekolwiek konia! <a href=stajnia.php>Wróć</a>");
    }
    if ($house -> fields['build'] == 0) {
        ...
        if ($house -> fields['points'] > 0) {
            $smarty -> assign ("Upgrade", "<a href=stajnia.php?action=build&step=upgrade>Nakarm Konia</a><br>");
        }
    }
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'add') {
        ...
        error ("Potrenowałes niece ze swoim koniem, urusł o 1, ale stracił on nieco na wartości. <a href=stajnia.php>Wróć</a>");
    }
...
if (isset ($_GET['action']) && $_GET['action'] == 'rent') {
 ...
 $db -> Execute("INSERT INTO log (owner, log, czas) VALUES(".$buy -> fields['seller'].",'<b><a href=view.php?view=".$player -> id.">".$player -> user."</a></b> zaakceptował twoj± ofertę za Konia. Dostałeś <b>".$buy -> fields['cost']."</b> sztuk złota do banku.','".$newdate."')") or error("Nie mogę dodać do dziennika.");
...
if (isset ($_GET['action']) && $_GET['action'] == 'my') {
 ...
 if ($player -> id == $house -> fields['owner']) {
     $smarty -> assign("Sellhouse", "- <a href=\"stajnia.php?action=my&step=sell\">Wystaw Konia na sprzedarz</a><br />");
 } else {
...
// Wystawianie konia na sprzedaż
 ...
 if (isset($_GET['step2']) && $_GET['step2'] == 'sell') {
     ...
     $db -> Execute("UPDATE houses SET cost=".$_POST['cost']." WHERE id=".$house -> fields['id']);
     $db -> Execute("UPDATE houses SET seller=".$player -> id." WHERE id=".$house -> fields['id']);
     $db -> Execute("UPDATE houses SET owner=0 WHERE id=".$house -> fields['id']);
  ...
  $smarty -> assign("Message", "Usun±łeś współwłaściciela konia");
     }
 }
    }
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'bedroom') {
     ...
     if ($player -> id == $house -> fields['owner']) {
                if ($house -> fields['rest'] == 'Y') {
                    error ("<br /><br />Możesz jeĽdzić tylko raz na reset!");
                }
  ...
  $cape -> Close();
                ...
                error ("<br><br>JeĽdziłes jakiś czas i odzyskałeś ".$gainenergy." energii, ".$gainlife." punktów życia oraz ".$gainmagic." punktów magii");
            } else {
                error ("<br><br>Próbowałeś nieco odpocz±ć ale przeszkadzał ci w tym Koń. Niestety nie udało ci się odpocz±ć");
            }
        }
    }
 ...
 $amount -> Close();
  ...
  $test -> Close();
         ...
         error ("Wzi±łeś z domu ".$_POST['amount']." sztuk(i) ".$zbroj -> fields['name']);
            }
        }
     ...
     $arritem -> Close();
            ...
            if (isset ($_GET['step3']) && $_GET['step3'] == 'add') {
                if (!isset($_POST['przedmiot'])) {
                    error("Podaj który przedmiot chcesz wzi±ść!");
                }
