  ...
  if ($szansa<2)
{
	 ...
	 if ($roll == 1)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.", '<font color=yellow>&#34;Sjerony&#34;</font>', 500, 'L', 5000, '-500', 1000, 30, 1000, 1, 'N', 0, 500, 'N')") or die ($db->ErrorMsg());
     $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
	 }
     if ($roll == 2)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.", '<font color=yellow>&#34;Insenviv&#34;</font>', 300, 'B', 500000, '-500', 1000, 30, 1000, 1, 'N', 0, 600, 'N')");
     $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
      ...
      if ($roll == 3)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Krisux&#34;</font>', 500,'W',500000,'-400',1000,30,1000,1,'N',0,500,'N')");
     $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
      if ($roll == 4)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Avurt&#34;</font>', 600,'A',500000,'-500',1000,30,1000,1,'N',0,500,'N')");
    $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
      if ($roll == 5)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Humar&#34;</font>', 500,'H',500000,'-500',1000,30,1000,1,'N',0,500,'N')");
     $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
      if ($roll == 6)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Rytus&#34;</font>', 500,'S',500000,'-500',1000,30,1000,1,'N',0,500,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
      if ($roll == 7)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Nutiduny&#34;</font>', 800,'L',500000,'-800',1000,60,1000,1,'N',0,800,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
     if ($roll == 8)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Szuit&#34;</font>', 800,'B',500000,'-850',1000,60,1000,1,'N',0,900,'N')");
    $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
      
      if ($roll == 9)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Bioluh&#34;</font>', 900,'W',500000,'-910',1000,60,1000,1,'N',0,950,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id); 
     }
      if ($roll == 10)
     {
  
        $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Durizjum&#34;</font>', 666,'A',500000,'-999',1000,60,1000,1,'N',0,999,'N')");;
    $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
      if ($roll == 11)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Hunajan&#34;</font>', 1000, 'H', 500000, '-900', 1000, 60, 1000, 1, 'N', 0, 950, 'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id); 
     }
      if ($roll == 12)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Claritun&#34;</font>', 900,'S',500000,'-850',1000,60,1000,1,'N',0,1100,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
     
       if ($roll == 13)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Opiena&#34;</font>', 3000,'C',500000,0,0,30,0,1,'N',0,0,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
        if ($roll == 14)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Siumba&#34;</font>', 50000,'C',500000,0,0,60,0,1,'N',0,0,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
      ...
      if ($roll == 15)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Zunia&#34;</font>', 2,'T',500000,0,0,30,0,1,'N',0,0,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
        if ($roll == 16)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohandd) VALUES(".$player -> id.",'<font color=yellow>&#34;Munda&#34;</font>', 1,'T',500000,0,0,60,0,1,'N',0,0,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
         if ($roll == 17)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Sylertusy&#34;</font>', 800,'R',5,0,10000,30,10000,1,'N',0,0,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }
            if ($roll == 18)
     {
          $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand) VALUES(".$player -> id.",'<font color=yellow>&#34;Gradorsy&#34;</font>', 2000,'R',5,0,1000000,60,1000000,1,'N',0,0,'N')");
   $db -> Execute("UPDATE players SET rary=rary+1 WHERE id=".$player -> id);
     }

     
     $tekst=" oraz zdobywa tajemniczy przedmiot! (nr:".$roll.")";
}
else
{
     $tekst='';
	...
	if ($fighter -> gainexp > 0 && $ucieczka == 0) {
	 ...
	 $db -> Execute("UPDATE players SET bosswalki=bosswalki+1 WHERE id=".$player -> id);
		}
	...
	if ($title !== 'Arena Walk' && $fighter -> hp == 0) {
        ...
        $db -> Execute("UPDATE players SET boss=boss+1 WHERE id=".$player -> id);
