		...
		if ($rzut < $attacker -> criticalhit && !$attacker -> noweapon) {
			...
			if ($rzut < 60) {
				$damage += 2 * $attacker -> criticalstat;
					$strMessage .= '<b>'.$attacker -> user.'</b>: szansa na trafienie krytyczne! ('.$attacker->criticalhit.' przeciwko: '.$rzut2.')<br>';
					$strDebug .= '<b>'.$attacker -> user.'</b>: szansa na trafienie krytyczne! ('.$attacker->criticalhit.' przeciwko: '.$rzut2.')<br><i>Wynik rzutu:'.$rzut.', wyliczenie:'.$attacker->criticalhit.'</i><br>';
				}
			else if ($rzut < 90) {
			...
			//one final shot
				...
				//gain attack
					...
					$strDebug .= '<i>Zysk 0.05 walki bronią</i><br>';
				}
				else if ($attacker -> usebow) {
				//gain shoot
					$attacker -> gainshoot += 0.05;
					$strDebug .= '<i>Zysk 0.05 strzelectwa</i><br>';
				}
				else if ($attacker -> usespell) {
				//gain magic
					$attacker -> gainmagic += 0.05;
					$strDebug .= '<i>Zysk 0.05 rzucania czarów</i><br>';
				}
				$eor=1;
				}
			}
...
//pech maga
	if (!$eor && $attacker -> usespell) {
		...
		if ($rzut < $attacker -> misspell) {
				...
				$strDebug .= '<b>'.$attacker -> user.'</b> koncentracja: *niepowodzenie* (-<b>'.$attacker -> usespell.'</b> many)<br /><i>Rzut:'.$rzut.', wyliczenie:'.$attacker->misspell.'<i><br>';
				$eor=1;
				}
...
function dofight(&$attacker, &$defender)
{
	...
	$strMessage = '';
	$round = 0;
	print("Att:".$attacker->debug."<br/>"."Def:".$defender->debug."</br></br>");
