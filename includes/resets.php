...
function start()  /// Things that take place during both resets.
{
    ...
    $db -> Execute("UPDATE kon SET points=points+2, rest='N', locrest='N'");
    
    $db -> Execute("UPDATE players SET wskrzes=0, zrodlo=0, przygodaa=0");
    $db -> Execute("UPDATE players SET brama=0, burdel=0, precyzja=0");
    $db -> Execute("UPDATE players SET transmutacja='brak', los=0 ");
            
    $db -> Execute("UPDATE gielda SET ilosc=40");
    $db -> Execute("UPDATE gielda SET cena+30");
    $db -> Execute('UPDATE tribes SET godziny=godziny-1 WHERE lekarz>1 AND godziny>0');  
    $db -> Execute("UPDATE tribeblogoslawienstwa SET sila=0, zrecznosc=0, szybkosc=0, inteligencja=0, silawoli=0, wytrzymalosc=0, kowalstwo=0, jubilerstwo=0, alchemia=0, zielarstwo=0, gornictwo=0, hutnictwo=0, stolarstwo=0, blogoo=0, drwalnictwo=0, umiejka=0");



$db -> Execute("UPDATE players SET nov=0, eof=0, voe=0, vii=0, rut=0, sta=0, wej=0");
$db -> Execute('UPDATE runy SET vii="<font color=red>NIEAKTYWNA</font>"');
$db -> Execute('UPDATE runy SET voe="<font color=red>NIEAKTYWNA</font>"');
$db -> Execute('UPDATE runy SET eof="<font color=red>NIEAKTYWNA</font>"');
$db -> Execute('UPDATE runy SET rut="<font color=red>NIEAKTYWNA</font>"');
$db -> Execute('UPDATE runy SET nov="<font color=red>NIEAKTYWNA</font>"');
$db -> Execute('UPDATE runy SET wejscie=0');


    //$db -> Execute("UPDATE players SET houserest='N'");
	// Delete inkeeper's gossips about players.
	$db -> Execute('TRUNCATE TABLE `events`');
	// Grow herbs.
	$db -> Execute('UPDATE `farm` SET `age`=`age`+1');
	$db -> Execute('DELETE FROM `farm` WHERE `age`>37');
	...
	$arrStats = $db -> GetAll('SELECT `id`, `klasa`, `inteli`+`wisdom`, `antidote_d`, `antidote_n`, `antidote_i`, `resurect` FROM `players` WHERE `klasa`="Mag" ');
	...
	$db -> Execute('UPDATE `players` SET `crime`=`crime`+1, `astralcrime`=\'Y\' WHERE (`klasa`=\'Złodziej\' OR `podklasa`=\'Łotr\') AND `freeze`=0');
	$db -> Execute('UPDATE `players` SET `pw`=`pw`+2 WHERE `deity`!=\'\'');
	// Various other actions.
	$db -> Execute('UPDATE `tribes` SET `atak`=\'N\'');
	$db -> Execute('UPDATE `outposts` SET `turns`=`turns`+3, `fatigue`=100, `attacks`=0');
...
function mainreset()
{
	...
	global $arrLanguage;
	/*
	$test=$db->Execute("SELECT value FROM settings WHERE setting='resets' ");
	$reset_factor=24 - $test->fields['value'];
	*/
	start();
	...
	$db -> Execute('UPDATE `players` SET `age`=`age`+1, `houserest`=\'N\', boss=0, houserest="N", chaos=0, gotow=0, zrodlo="0", przygodaa="0", transmutacja="brak" ');
	$db -> Execute('UPDATE `woodfarm` SET `age`=`age`+1');
	$db -> Execute('UPDATE `players` SET `kupmuttau`= 0');
	$db -> Execute('UPDATE `players` SET `pktwampira`=`pktwampira`+7, `pktszpiega`=`pktszpiega`+3');
	$db -> Execute('UPDATE `players` SET  wodka="0", gotow="0", nughorgbicie="0", precyzja=0, pilem=0');
    ...
    $db -> Execute("UPDATE houses SET points=points+2");
          $db -> Execute("UPDATE players SET brama=0");
           $db -> Execute("UPDATE players SET bosszabity=0");
           $db -> Execute("UPDATE players SET kluczaktywny=0");
           $db -> Execute("UPDATE players SET losowanienagrody=0");
           $db -> Execute("UPDATE players SET najemnikblokada=0");
	$db -> Execute("UPDATE kon SET points=points+2");
	$db -> Execute("UPDATE gielda SET ilosc=40, cena=cena+30");
   $db -> Execute("UPDATE tribes SET ladunki=0, blok='1'");
     
  //dodaj brakującą enę i punkty z niewykonanych restów zwykłych
  /*
	$db -> Execute('UPDATE `players` SET `energy`=`energy`+`max_energy`*'.$reset_factor.', `hp`=`max_hp`, `bridge`=\'N\' WHERE `miejsce`!=\'Lochy\' AND `freeze`=0 AND `rasa`!=\'\' AND `klasa`!=\'\'');
	$db -> Execute('UPDATE `players` SET `energy`=63*`max_energy` WHERE `energy` > 63 * `max_energy` AND `max_energy`!=0');
	$db -> Execute('UPDATE `players` SET `crime`=`crime`+1*'.$reset_factor.', `astralcrime`=\'Y\' WHERE (`klasa`=\'Złodziej\' OR `podklasa`=\'Łotr\') AND `freeze`=0');
	$db -> Execute('UPDATE `players` SET `pw`=`pw`+2*'.$reset_factor.' WHERE `deity`!=\'\'');
	// Various other actions.
	$db -> Execute('UPDATE `tribes` SET `atak`=\'N\'');
	$db -> Execute('UPDATE `outposts` SET `turns`=`turns`+2*'.$reset_factor.', `fatigue`=100, `attacks`=0');
	$db -> Execute('UPDATE `outposts` SET `turns`=`turns`+1*'.$reset_factor.' WHERE `size`>49 AND `warriors`+`archers` > `size` / 10');
	$db -> Execute('UPDATE `outposts` SET `turns`=`turns`+1*'.$reset_factor.' WHERE `size`>99 AND `warriors`+`archers` > `size` / 4');
	$db -> Execute('UPDATE `outposts` SET `turns`=`turns`+1*'.$reset_factor.' WHERE `size`>149 AND `warriors`+`archers` > `size` / 2');

	// Jail - count duration and free prisoners.
	$db -> Execute('UPDATE `jail` SET `duration`=`duration`-1*'.$reset_factor);
	$arrJail = $db -> GetAll('SELECT `prisoner` FROM `jail` WHERE `duration`<=0');
	for ($i = 0, $intMax = count($arrJail); $i < $intMax; ++$i)
		$db -> Execute('UPDATE `players` SET `miejsce`=\'Altara\' WHERE `id`='.$arrJail[$i][0]);
	$db -> Execute('DELETE FROM `jail` WHERE `duration`<=0');
	unset($arrJail);
	$db -> Execute('UPDATE `chat_config` SET `resets`=`resets`-1*'.$reset_factor);
	$db -> Execute('DELETE FROM `chat_config` WHERE `resets`<=0');
	
	$db-> Execute('UPDATE settings SET value=0 WHERE setting="resets" ');
	//wyslij loga do każdego że zostało wykonane więcej restów.
	*/
...
/// Variant 2: Not enough data. Add new reset copying the amount from old one (if it's day 1 of new era, use base values).
	...
	///Warehouse minerals recomplete - only on sunday. Disabled now
	/*
	$now=getdate();
	if ($now["wday"] ==0)
	{
		...
		{
			$arrAdd[$i]=rand(250,1200);
			$db->Execute('UPDATE warehouse SET amount=amount+'.$arrAdd[$i].' WHERE mineral=\''.$arrItems[$i].'\'');
		}
	...
	*/
	...
	$arrMinerals = array('coal' => 0.75,
						 ...
						 'tin' => 3,
						 'iron' => 4);
	$arrMaxDeposit = array(1000,3000,6000);  // max amount of ore based on day 1,2 or 3.
	for ($i = 0, $intMax = count($arrSearchmin); $i < $intMax; ++$i)
	{
		$strMineral = $arrSearchmin[$i][1];
		$intAmount = ceil(rand(1, 10) * $arrMaxDeposit[$arrSearchmin[$i][2] - 1] / $arrMinerals[$strMineral])*2;
	...
	if ($arrTest[0] == '')
	{
		...
		for ($i = 0, $intMax = count($arrAstral); $i < $intMax; ++$i)
		{
			...
			$db -> Execute('UPDATE `astral_machine` SET `used`=used+directed, `directed`=0 WHERE `owner`='.$arrAstral[$i][0]);
			if ($blnBreak)
				break;
		}
	}
	...
	//cores
	$db -> Execute("UPDATE tribeblogoslawienstwa SET ladunki=0");
	 $db -> Execute("UPDATE tribes SET ladunki=0");
	
	$db -> Execute('UPDATE `coresplayers` SET `age` = `age` + 1');
	$db -> Execute('UPDATE `coresplayers` SET  `rest` = `rest` - 1 WHERE `rest` > 0');
  $db -> Execute('UPDATE players SET los=0'); $db -> Execute("UPDATE houses SET points=points+2");
  ...
  //lottery
	...
	$day=getdate();
	$day=$day['mday'];
	$db -> Execute("update settings set value='".$arrstring."' where setting='lastlos'");
	$db -> Execute("update settings set value=".$day." where setting='lastlosdate'");
	
	 $db -> Execute("UPDATE tribes SET ladunki=0");
	  
  
  //zip log file for history sake
	$zip= new ZipArchive();
	$plik= "./log_archive/".date("y-m-d").".zip";
	if ($zip->open($plik,ZIPARCHIVE::CREATE) !==true)
	{
		$zip -> close();
		die("nie mogę zrobić pliku");
	}
	$zip->addFile("ourlogs.txt");
	$zip->addFile("ourlogs.csv");
	$zip -> close();
	unlink("ourlogs.txt");
	unlink("ourlogs.csv");
...
function smallreset()
{
	start();
	//reset counter
	//$db->Execute("UPDATE settings SET value=value+1 WHERE setting='resets' ");
	stop();
}
$time_stop=time();
    print('wykonanie:'.$time_stop-$time_start);
?>
