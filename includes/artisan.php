...
function makeastral2($planId = 0)
{
    ...
    else
    {
        ...
        $arrExp = array(200000, 300000, 400000, 500000, 700000);
...
function astralmake($planId, $strPlan, $strPlan2, $fltSkill, $strSkill, &$arrResources, &$arrAmount, &$arrNames, &$arrSqlResources, &$arrExp)
{
	...
	{
		$objMinerals = mysql_query('SELECT `'.implode("`, `", $arrSqlResources).'` FROM `minerals` WHERE `owner`='.$player -> id);
	}
	else
	{
		$objMinerals = mysql_query('SELECT `'.implode("`, `", $arrSqlResources).'` FROM `herbs` WHERE `gracz`='.$player -> id) or die(mysql_error());
	}
    //$objMinerals = ($strSkill != 'alchemia' ? mysql_query('SELECT `'.implode("`, `", $arrSqlResources).'` FROM `minerals` WHERE `owner`='.$player -> id) : mysql_query('SELECT illani, nutari, illanias, dynallca FROM `herbs` WHERE `owner`='.$player -> id));
    $objMins = mysql_fetch_array($objMinerals);
    ...
    for ($i = 0; $i< $intCountMinerals; $i++)
    {
		...
		if ($objMins[$arrSqlResources[$i]] < $arrAmount[$intKey][$i])
        {
    ...
    if (rand(1, 100) <= $intChance) /// success
    {
        ...
        $objTest -> Close();
        $intGainexp = rand(1, 10000) + $arrExp[$intKey];
    ...
    else
    {
		...
		$db -> Execute($strTakeMinerals.' WHERE `gracz`='.$player -> id);
    }
    //$db -> Execute($strTakeMinerals.' WHERE `owner`='.$player -> id);
}
