...
if (isset($_GET['atak']) && (isset($_GET['step3']) && $_GET['step3'] == 'confirm'))
{
    ...
    if ($matak >= $eobrona)
    {
        ...
        $smarty -> assign(array("Myname" => $mytribe -> fields['name'],
                                ...
                                "Astral" => $strComponent));
        $db -> Execute("UPDATE `tribes` SET `credits`=`credits`+".$gzloto.", `platinum`=`platinum`+".$gmith.", `wygr`=`wygr`+1, `prest`=`prest`+5 WHERE `id`=".$mytribe -> fields['id']);
        $db -> Execute("UPDATE `tribes` SET `credits`=`credits`-".$gzloto.", `platinum`=`platinum`-".$gmith.", `przeg`=`przeg`+1, `prest`=`prest`-3 WHERE `id`=".$klan -> fields['id']);
        $db -> Execute("UPDATE `gielda` SET `cost`=`cost`+30 WHERE `id`=".$mytribe -> fields['id']);
        $db -> Execute("UPDATE `gielda` SET `cost`=`cost`-30 WHERE `id`=".$klan -> fields['id']);
        ...
        else
    {
        ...
        $objPerm -> Close();
        $db -> Execute("UPDATE `tribes` SET `credits`=`credits`-".$gzloto.", `platinum`=`platinum`-".$gmith.", `przeg`=`przeg`+1, `prest`=`prest`-3 WHERE `id`=".$mytribe -> fields['id']);
        $db -> Execute("UPDATE `tribes` SET `credits`=`credits`+".$gzloto.", `platinum`=`platinum`+".$gmith.", `wygr`=`wygr`+1, `prest`=`prest`+5 WHERE `id`=".$klan -> fields['id']);
    }
}
?>
