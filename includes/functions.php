...
*/
function CopyEquip($id)
{
		
		$query="INSERT INTO equipment SELECT NULL, owner, name, power, status, type, cost, minlev, zr, wt, szyb, maxwt, magic, poison, amount, twohand, NULL, ptype, repair, NULL, wzmocnienie, ulepszeniee FROM equipment WHERE id=".$id;
...
function RepairEquip($id)
{
		...
		$query="INSERT INTO equipment SELECT NULL, owner, name, power, status, type, cost, minlev, zr, maxwt, szyb, maxwt, magic, poison, amount, twohand, NULL, ptype, repair, NULL, wzmocnienie, ulepszeniee FROM equipment WHERE id=".$id;
 ...
 function drink($id)
 {
    ...
    if ($strType == 'A' && $intRoll > 50 && !isset($message))
    {
            ...
            elseif (ereg("Illani",$miks -> fields['name']))
        {
            ...
            elseif (ereg("wskrzeszenia",$miks -> fields['name']))
        {
            $pwnd= $miks -> fields['power'];
	    $db -> Execute("UPDATE `players` SET `resurect`=".$pwnd." WHERE `id`=".$player -> id);
            $efekt = GAIN_RESURECT;
        }
        
    }
	if ($strType == 'B' && $intRoll > 50 && !isset($message))
    {
        if (ereg("siły",$miks -> fields['name']))
        {
            $test=$db->Execute("Select * from effects where effect='siła' and source='P' and owner=".$player->id);
			if ($test->fields['id'])
			{
				$db->Execute("Update effects set effect_power=effect_power+".$miks->fields['power']." where effect='siła' and source='P' and owner=".$player->id);
			}
			else
			{
				$db->Execute("Insert into effects (owner, effect, effect_power, source) VALUES (".$player->id.", 'siła', ".$miks->fields['power'].", 'P')");
			}
            $efekt = "Zyskałeś tymczasową premię do siły. Premia będzie trwać do następnej walki lub do następnego resetu.";
        }
...
function equip ($id)
{
    ...
    if ($player -> clas == 'BarbarzyĹca' && ($equip -> fields['magic'] == 'Y' || ($equip -> fields['type'] == 'I' && $equip -> fields['power'])))
    {
    ...
    if ($type == 'R')
    {
        ...
        {
            $db -> Execute("INSERT INTO `equipment` (`name`, `wt`, `power`, `status`, `type`, `owner`, `poison`, `ptype`, ulepszeniee) VALUES('".$equip -> fields['name']."',".$wt.",".$equip -> fields['power'].",'E','R',".$player -> id.",".$equip -> fields['poison'].",'".$equip -> fields['ptype']."',".$equip->fields['ulepszeniee'].")") or error(E_ARROWS);
