...
*/
function checkexp ($currentExp,$gainedExp,$currentLevel,$race,$user,$eid,$enemyid,$enemyuser,$playerid,$skill,$amount,$prest) 
{
    global $player;
    global $tribes;
    global $db;
    ...
    $arrRaces = array('Człowiek', 'Elf', 'Krasnolud', 'Hobbit', 'Jaszczuroczłek', 'Gnom', 'Półsmok', 'Kobold', 'Demon', 'Półent', 'Cień', 'Mroczny Elf', 'Nieumarły', 'Tauren', 'Umarlak', 'Ork', 'Paladyn', 'Górski Ork', 'Ogr');
    $arrHP = array(3, 2, 4, 2, 5, 3, 5, 3, 4, 4, 4, 4, 3, 5, 2, 4, 3, 5, 3, 4);
    $intKey = array_search($race, $arrRaces);
    $fltEnergyAmount = $player -> clas == 'Rzemieślnik' ? 0.5 : 0.3;

    $intLevelExp = round(pow($currentLevel, 2) * log(exp(1) + (($currentLevel - 1) / 100)) * 50);
    while($intTempExp >= $intLevelExp)
    {
        $intAP += 10 + floor(.10 * $currentLevel);
    ...
    if ($intGainedLevel > 0) 
    {
        if ($playerid == $eid) 
        {
            ...
            if ($player -> tribe !='')
			{
				if ($player -> clas == 'Rzemieślnik')
				{
					$gainedprest=$intGainedLevel * 20;
				}
				else
				{
					$gainedprest=$intGainedLevel * 10;
				}
			$strText .="<br>".$gainedprest." prestiżu dla swojej gildii";
			$db -> Execute("UPDATE tribes SET prest=prest+".$gainedprest." WHERE id=".$player -> tribe);
			$db -> Execute("UPDATE `gielda` SET `cost`=`cost`+150 WHERE `id`=".$player -> tribe);
			}
			print $strText;
        }
