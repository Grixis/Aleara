    ...
    else
{
    ...
    */
	...
	$no_account=0;
	//from fb?
	if ($_GET['from'] == 'facebook')
	{
		print("ID usera:".$_GET['fbid']);
		setcookie("fbid",$_GET['fbid']);
		
		$test=$db->Execute("SELECT `id`, `email`, `pass` from `players` WHERE `fb-id`=".$_GET['fbid']);
		if (!$test->fields['id'])
		{
			$no_account=1;
		}
		...
		{
			setcookie("fbuser", $test->fields['email']);
			setcookie("fbkey", $test->fields['pass']);
			
			$smarty->assign("Fbuser", $test->fields['email']);
			$smarty->assign("Fbkey", $test->fields['pass']);
			
		}
		$facebook=1;
		$smarty->assign("Fbid", $_GET['fbid']);
