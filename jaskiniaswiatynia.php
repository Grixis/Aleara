...
if ($player -> level < 25)
    {
        error("Dotarłeś do świątyni w jaskini. jednak możesz tutaj wejść mając lvl 25-50! <a href=puszcza.php> (Wróć pod bramę)</a>");
    }
...
if(isset($_POST['submit']))
{
    switch($_POST['item'])
    {
       case 'credits':
            $c = 1;
            $n = "złoto +10000<br>mithril +30<br>exp +1500<br>energia +5<br>";
            ...
            $db -> Execute("UPDATE players SET platinum=platinum+30 WHERE id =".$player -> id);
            
            $db -> Execute("UPDATE players SET energy=energy+5 WHERE id =".$player -> id);
            checkexp($player -> exp, 1500, $player -> level, $player -> race, $player -> user, $player -> id, 0, 0, $player -> id, 'fletcher', 0);
       break;
    }
    ...
    $db -> Execute("UPDATE players SET kjaskini=kjaskini-1 WHERE id=".$player -> id);
...
*/
$smarty -> display ('jaskiniaswiatynia.tpl');
