...
if (isset ($_GET['buy'])) 
{
    ...
    if (!$test -> fields['id']) 
    {
        $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, szyb, twohand, repair, icon) VALUES(".$player -> id.",'".$arm -> fields['name']."',".$arm -> fields['power'].",'B',".$newcost.",".$arm -> fields['zr'].",".$arm -> fields['maxwt'].",".$arm -> fields['minlev'].",".$arm -> fields['maxwt'].",1,".$arm -> fields['szyb'].",'Y', ".$arm -> fields['repair'].", '".$arm->fields['icon']."')") or error(E_DB);
    } 
        else 
    {
...
if (isset($_GET['arrows']))
{
    ...
    if (isset($_GET['step']) && $_GET['step'] == 'buy')
    {
        ...
        if ((isset($_POST['arrows1']) && $_POST['arrows1'] > 0) || (isset($_POST['arrows2']) && $_POST['arrows2'] > 0))
        {
            ...
            if (!$test -> fields['id']) 
            {
                $db -> Execute("INSERT INTO equipment (owner, name, power, cost, wt, szyb, minlev, maxwt, type, icon) VALUES(".$player -> id.",'".$objArm -> fields['name']."',".$objArm -> fields['power'].",".$intNewcost.",".$intAmount.",".$objArm -> fields['szyb'].",".$objArm -> fields['minlev'].",".$intAmount.",'".$objArm -> fields['type']."', '".$objArm ->fields['icon']."')") or error(E_DB);
            } 
                else 
            {
