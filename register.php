...
if ($player -> rank == 'Admin' or $player -> rank == 'Staff' or $player -> rank == "Kr�lowa Pot�pionych")
{
$add="<a href='admin.php?view=weather'>[Zmie� pogod�]</a>";
}
else
{
$add="";
...
*/ 
    ...
    $strPass = MD5($_POST['pass']);
    $db -> Execute("INSERT INTO players (user, email, pass) VALUES(".$strUser.", ".$strEmail.", '".$strPass."' )") or die($db -> ErrorMsg());
    $lastid=$db->Execute("Select id from players order by id desc limit 1");
    $db->Execute("INSERT INTO mail (sender, senderid, owner, subject, body, date) VALUES ('Herold',0,".$lastid->fields['id'].",'Witaj w krainie!','".WELCOMEMSG."',NOW()") or die($db -> ErrorMsg());
    $smarty->assign("Message","INSERT INTO mail (sender, senderid, owner, subject, body, date) VALUES ('Herold',0,".$lastid->fields['id'].",'Witaj w krainie!','".WELCOMEMSG."',NOW()");
    /**
    * Players they`ve ever been registered there
    */ 
    include("counter/usersever.php");
