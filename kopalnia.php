...
if (isset($_GET['action']) && $_GET['action'] == 'dig')
{
    ...
    $fltAbility = abilitybonus('mining') + $gornictwo + $test -> fields['gornictwo'];
    ...
    for ($i = 0; $i < $_POST['amount']; $i++)
    {
        ...
        if ($intRoll == 5)
        {
            $intAmount = max(1, ceil((rand(10, 30) * 3 / 8) * (1 + ($fltAbility + $fltGainAbility + $gornictwo + $test -> fields['gornictwo']) / 40)));
			$intAmount = ceil($intAmount * 0.5);
            $intGainExp += ceil($player -> level * 5);
            $arrMinerals[0] += $intAmount; // crystal
        }
        if ($intRoll == 6)
        {
            $intAmount = max(1, ceil((rand(1, 20) * 2 / 5) * (1 + ($fltAbility  + $fltGainAbility + $gornictwo + $test -> fields['gornictwo']) / 40)));
			$intAmount = ceil($intAmount * 0.5);
            $intGainExp += ceil($player -> level * 5);
        ...
        {
            $intAmount = max(1, ceil((rand(1, 20) * 1 / 3) * (1 + ($fltAbility + $fltGainAbility + $gornictwo + $test -> fields['gornictwo']) / 40)));
			$intAmount = ceil($intAmount * 0.5);
            $intGainExp += ceil($player -> level * 5);
        ...
        if ($intRoll == 9)
        {
            $intAmount = max(1, ceil(rand(50, 200) * (1 + ($fltAbility + $fltGainAbility + $gornictwo + $test -> fields['gornictwo']) / 40)));
			$intAmount = ceil($intAmount * 0.5);
            $arrGold[0] += $intAmount; // gold
        }
        if ($intRoll == 10)
        {
            $intLostHP = rand(1, 150);
            if ($intLostHP < 51)
            {
