...
print('<span id="player" style="display: none;">'.$player->id .'</span>');

$test2 = $db -> Execute("SELECT szyb FROM equipment WHERE type='A' AND status='E' AND owner=".$player -> id);
$szyb = $test2 -> fields['szyb'];
$smarty -> assign(array("szyb" => $szyb));

$zr = $db -> Execute("SELECT zr FROM equipment WHERE type='B' AND status='E' AND owner=".$player -> id);
$luk = $zr -> fields['zr'];
$smarty -> assign(array("zr" => $zr));

$test = $db -> Execute("SELECT szyb FROM equipment WHERE type='H' AND status='E' AND owner=".$player -> id);
$helmspeed = $test -> fields['szyb'];
$smarty -> assign(array("helmspeed" => $helmspeed));
$test = $db -> Execute("SELECT zr FROM equipment WHERE type='H' AND status='E' AND owner=".$player -> id);
$helmzr = $test -> fields['zr'];
$smarty -> assign(array("helmzr" => $helmzr));
$test = $db -> Execute("SELECT tarczaspeed FROM equipment WHERE type='H' AND status='E' AND owner=".$player -> id);
$tarczaspeed = $test -> fields['tarczaspeed'];
$smarty -> assign(array("tarczaspeed" => $tarczaspeed));
$test = $db -> Execute("SELECT szyb FROM equipment WHERE type='L' AND status='E' AND owner=".$player -> id);
$nagolespeed = $test -> fields['szyb'];
$smarty -> assign(array("nagolespeed" => $nagolespeed));
$test = $db -> Execute("SELECT power FROM equipment WHERE type='H' AND status='E' AND owner=".$player -> id);
$power = $test -> fields['power'];
$smarty -> assign(array("power" => $power));
...
/**
* Function to calculate repair cost of an item
*/
function repaircost($strName, $intItemlevel)
{
	if (strpos($strName, "z miedzi") !== false) return $intItemLevel * 1;
	else if (strpos($strName, "z brązu") !== false) return $intItemLevel * 4;
	else if (strpos($strName, "z mosiądzu") !== false) return $intItemLevel * 16;
	else if (strpos($strName, "z żelaza") !== false) return $intItemLevel * 64;
	else if (strpos($strName, "ze stali") !== false) return $intItemLevel * 256;
	else if (strpos($strName, "z leszczyny") !== false) return $intItemLevel * 2;
	else if (strpos($strName, "z cisu") !== false) return $intItemLevel * 8;
	else if (strpos($strName, "z wiązu") !== false) return $intItemLevel * 32;
	else if (strpos($strName, "wzmocniony") !== false) return $intItemLevel * 128;
	else if (strpos($strName, "kompozytowy") !== false) return $intItemLevel * 512;
	else return $intItemLevel * 256;
}
/**
...
function backpack($type,$playerid,$nameitems,$type2,$smartyname)
{
    ...
    while (!$arm -> EOF)
    {
        ...
        {
            $arrshow[$j] = "<b>(".AMOUNT.": ".$arm -> fields['amount']." )</b> ".$arm -> fields['name']." (+".$arm -> fields['power']." siły czarów) [ <a href=\"equip.php?equip=".$arm -> fields['id']."\">".A_WEAR."</a> | <A href=\"equip.php?sell=".$arm -> fields['id']."\">".A_SELL."</a> ".FOR_A." ".$arm -> fields['cost']." ".GOLD_COINS." ]<br />";
        }
            else
        {
...
if (isset($_GET['schowaj']))
{
    ...
    if ($bron -> fields['status'] != 'U' and $bron -> fields['status'] !='E')
    {
	error (ERROR);
    }
        ...
        else
    {
        ...
        {
            $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand, ptype, repair, wzmocnienie, ulepszeniee) VALUES(".$player -> id.",'".$bron -> fields['name']."',".$bron -> fields['power'].",'".$bron -> fields['type']."',".$bron -> fields['cost'].",".$bron -> fields['zr'].",".$bron -> fields['wt'].",".$bron -> fields['minlev'].",".$bron -> fields['maxwt'].",1,'".$bron -> fields['magic']."',".$bron -> fields['poison'].",".$bron -> fields['szyb'].",'".$bron -> fields['twohand']."', '".$bron -> fields['ptype']."', ".$bron -> fields['repair'].", '".$bron -> fields['wzmocnienie']."',".$bron -> fields['ulepszeniee'].")") or error(E_DB);
        }
        $test -> Close();
...
if (isset($_GET['sell_potion']))
{
    ...
    $cost = $mik2 -> fields['illanias']/5 + $mik2 -> fields['illani']/5 + $mik2 -> fields['nutari']/5 + $mik2 -> fields['dynallca']/5;
...
if (isset($_GET['sell']))
{
	...
	if ($sell -> fields['status'] != 'U' and $sell -> fields['status'] !='E')
    {
		error (ERROR);
    }
    if ($player -> id != $sell -> fields['owner'])
...
if (isset($_GET['sell_allpotion']))
{
    ...
    while (!$cost_potion -> EOF)
    {
        ...
        $profit = $profit + ($mik2 -> fields['illanias']/5 + $mik2 -> fields['illani']/5  +$mik2 -> fields['nutari']/5 +$mik2 -> fields['dynallca']/5)* $cost_potion -> fields['amount'];
...
if (isset($_GET['napraw_uzywane']))
{
    ...
    while(!$rzecz_wiersz -> EOF)
    {
        ...
        {
            $repair=$rzecz_wiersz -> fields['maxwt'] - $rzecz_wiersz -> fields['wt'];
			if ($repair > $player -> credits)
            {
                error (NO_MONEY);
            }
            $player -> credits = $player-> credits - $repair;
            $db -> Execute("UPDATE equipment SET wt=".$rzecz_wiersz -> fields['maxwt']." WHERE id=".$rzecz_wiersz -> fields['id']);
            $db -> Execute("UPDATE players SET credits=credits-".$repair." WHERE id=".$player -> id);
            $text = $text."<br />".YOU_REPAIR." ".$rzecz_wiersz -> fields['name']." ".AND_COST." ".$repair." ".GOLD_COINS.".<br />";
        }
...
if (isset($_GET['napraw']))
{
    ...
    if ($player -> credits < $rzecz -> fields['maxwt'] - $rzecz -> fields['wt'])
    {
        error (NO_MONEY);
    }
    ...
    else
    {
        ...
        if (!$test -> fields['id'])
        {
            $db -> Execute("INSERT INTO equipment (`owner`, `name`, `power`, `type`, `cost`, `zr`, `wt`, `minlev`, `maxwt`, `amount`, `magic`, `poison`, `szyb`, `ptype`, `repair`, `twohand`, wzmocnienie, ulepszeniee) VALUES(".$player -> id.",'".$rzecz -> fields['name']."',".$rzecz -> fields['power'].",'".$rzecz -> fields['type']."',".$rzecz -> fields['cost'].",".$rzecz -> fields['zr'].",".$rzecz -> fields['maxwt'].",".$rzecz -> fields['minlev'].",".$rzecz -> fields['maxwt'].",1,'".$rzecz -> fields['magic']."',".$rzecz -> fields['poison'].",".$rzecz -> fields['szyb'].", '".$rzecz -> fields['ptype']."', ".$rzecz -> fields['repair'].", '".$rzecz -> fields['twohand']."','".$rzecz->fields['wzmocnienie']."',".$rzecz->fields['ulepszeniee'].")");
		}
            else
        {
            $db -> Execute("UPDATE `equipment` SET `amount`=`amount`+1 WHERE `id`=".$test -> fields['id']);
	...
	$repair=$rzecz -> fields['maxwt'] - $rzecz -> fields['wt'];
    $db -> Execute("UPDATE `players` SET `credits`=`credits`-".$repair." WHERE `id`=".$player -> id);
    $smarty -> assign ("Action", "<br />".YOU_REPAIR." <b>".$rzecz -> fields['name']."</b> ".FOR_A." <b>".$repair."</b> ".GOLD_COINS.".<br />\n(<a href=\"equip.php\">".REFRESH."</a>)<br />\n");
    $rzecz -> Close();
}
    ...
    if (isset($_GET['step']) && $_GET['step'] == 'poison')
    {
        ...
        if ($intRoll > 50)
        {
            ...
            if ($item -> fields['type'] == 'W')
            {
                ...
                if (!$test -> fields['id'] )
                {
                    $db -> Execute("INSERT INTO `equipment` (`owner`, `name`, `power`, `type`, `cost`, `zr`, `wt`, `minlev`, `maxwt`, `amount`, `magic`, `poison`, `szyb`, `ptype`, `repair`, `twohand`, wzmocnienie, ulepszeniee) VALUES(".$player -> id.",'".$name."',".$item -> fields['power'].",'W',".$item -> fields['cost'].",".$item -> fields['zr'].",".$item -> fields['wt'].",".$item -> fields['minlev'].",".$item -> fields['maxwt'].",1,'N',".$intPower.",".$item -> fields['szyb'].", '".$strPtype."', ".$item -> fields['repair'].", '".$item -> fields['repair']."','".$item->fields['wzmocnienie']."',".$item->fields['ulepszeniee'].")") or error($db -> ErrorMsg());
                }
                else
                {
                    $db -> Execute("UPDATE `equipment` SET `amount`=`amount`+1 WHERE `id`=".$test -> fields['id']);
            ...
            else
            {
                if ($item -> fields['wt'] <= 100)
                {
                    ...
                    if (!$test -> fields['id'] )
                    {
                        $db -> Execute("INSERT INTO `equipment` (`owner`, `name`, `power`, `type`, `cost`, `zr`, `wt`, `minlev`, `maxwt`, `amount`, `magic`, `poison`, `szyb`, `ptype`, `repair`, wzmocnienie, ulepszeniee) VALUES(".$player -> id.",'".$name."',".$item -> fields['power'].",'R',".$item -> fields['cost'].",".$item -> fields['zr'].",".$item -> fields['wt'].",".$item -> fields['minlev'].",".$item -> fields['maxwt'].",1,'N',".$intPower.",".$item -> fields['szyb'].", '".$strPtype."', ".$item -> fields['repair'].", '".$item->fields['wzmocnienie']."',".$item->fields['ulepszeniee'].")") or error($db -> ErrorMsg());
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (!$test -> fields['id'] )
                    {
                        $db -> Execute("INSERT INTO `equipment` (`owner`, `name`, `power`, `type`, `cost`, `zr`, `wt`, `minlev`, `maxwt`, `amount`, `magic`, `poison`, `szyb`, `ptype`, `repair`, wzmocnienie, ulepszeniee) VALUES(".$player -> id.",'".$name."',".$item -> fields['power'].",'R',".$item -> fields['cost'].",".$item -> fields['zr'].",
                        35 ,".$item -> fields['minlev'].", 35 ,1,'N',".$intPower.",".$item -> fields['szyb'].", '".$strPtype."', ".$item -> fields['repair'].", '".$item -> fields['repair']."','".$item->fields['wzmocnienie']."',".$item->fields['ulepszeniee'].")") or error($db -> ErrorMsg());
                    }
                    else
                    {
...
if (isset($_GET['fill']))
{
    ...
    $intAmount = 100 - $objArrows -> fields['wt'];
...
if ($player -> clas != 'Mag')
{
    ...
    if ($arrEquip[0][0])
    {
        ...
        $smarty -> assign ("Weapon", $strWarn."<b>".WEAPON.":</b> ".$arrEquip[0][1]." (+".$arrEquip[0][2]." dmg) (+<b>".-$zr -> fields['zr']."</b> zr) ".showpoisoned($arrEquip[0][8], $arrEquip[0][3])."(+".$arrEquip[0][7]." ".EQUIP_SPEED.") (".$arrEquip[0][6]."/".$arrEquip[0][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[0][0]."\">".HIDE_WEP."</a>][<a href=\"equip.php?napraw=".$arrEquip[0][0]."\">".A_REPAIR."</a>]<br />\n");
    }
        else
    {
    ...
    else
{
    ...
    if ($arrEquip[0][0])
    {
        $strWarn = ($arrEquip[0][6] < 11) ? '<blink>' : '';
        $strWarn1 =($arrEquip[0][6] < 11) ? '</blink>' : '';
        $smarty -> assign ("Weapon", $strWarn."<b>".WEAPON.":</b> ".$arrEquip[0][1]." (+".$arrEquip[0][2].")(+<b>".-$power -> fields['power']."</b> zr) ".(showpoisoned($arrEquip[0][8], $arrEquip[0][3]))." (+".$arrEquip[0][7]." ".EQUIP_SPEED.") (".$arrEquip[0][6]."/".$arrEquip[0][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[0][0]."\">".HIDE_WEP."</a>][<a href=\"equip.php?napraw=".$arrEquip[0][0]."\">".A_REPAIR."</a>]<br />\n");
    }
        else
    {
        if ($arrEquip[7][0])
        {
            $smarty -> assign ("Weapon", "<b>".STAFF.":</b> ".$arrEquip[7][1]." (+".$arrEquip[7][2]." siły czarów) [<a href=\"equip.php?schowaj=".$arrEquip[7][0]."\">".HIDE."</a>]<br />\n");
        }
            else
        {
            $smarty -> assign ("Weapon", "<b>".WEAPON.":</b> ".EMPTY_SLOT."<br />\n");
...
if ($arrEquip[2][5] < 0)
    {
        $arrEquip[2][5] = str_replace("-","",$arrEquip[2][5]);
        $agility666 = "(+".$arrEquip[2][5]." ".EQUIP_AGI.")";
    }
    ...
    $smarty -> assign ("Helmet", $strWarn."<b>".HELMET.":</b> ".$arrEquip[2][1]." (+".$arrEquip[2][2].")(+<b>$helmspeed</b> szyb) ".$agility666." (".$arrEquip[2][6]."/".$arrEquip[2][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[2][0]."\">".WEAR_OFF."</a>][<a href=\"equip.php?napraw=".$arrEquip[2][0]."\">".A_REPAIR."</a>]<br />\n");
}
    else
{
...
if ($player -> clas != 'Mag')
{
    if ($arrEquip[3][0])
    {
        ...
        $smarty -> assign ("Armor", $strWarn."<b>".ARMOR.":</b> ".$arrEquip[3][1]." (+".$arrEquip[3][2].")(+<b>$szyb</b> szyb) ".$agility." (".$arrEquip[3][6]."/".$arrEquip[3][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[3][0]."\">".WEAR_OFF."</a>][<a href=\"equip.php?napraw=".$arrEquip[3][0]."\">".A_REPAIR."</a>]<br />\n");
    }
        else
    {
...
{
    if ($arrEquip[3][0])
    $test = $db -> Execute("SELECT szyb FROM equipment WHERE owner=".$playerid." AND type='A' AND status='S'");
...
$smarty -> assign(array("szyb" => $szyb));
    {
        ...
        $strWarn = ($arrEquip[3][6] < 11) ? '<blink>' : '';
        $strWarn1 =($arrEquip[3][6] < 11) ? '</blink>' : '';
        $smarty -> assign ("Armor", $strWarn."<b>".ARMOR.":</b>".$szyb." ".$arrEquip[3][1]." (+".$arrEquip[3][2].") ".$agility." (".$arrEquip[3][6]."/".$arrEquip[3][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[3][0]."\">".WEAR_OFF."</a>][<a href=\"equip.php?napraw=".$arrEquip[3][0]."\">".A_REPAIR."</a>]<br />\n");
    }
       
    {
        if ($arrEquip[8][0])
        {
            $smarty -> assign ("Armor", "<b>".CAPE.":</b>".$szyb." ".$arrEquip[8][1]." (+".$arrEquip[8][2]." % ".EQUIP_MANA.") [<a href=\"equip.php?schowaj=".$arrEquip[8][0]."\">".WEAR_OFF."</a>][<a href=\"equip.php?napraw=".$arrEquip[8][0]."\">".A_REPAIR."</a>]<br />\n");
        }
            else
        {
            $smarty -> assign ("Armor", "<b>".ARMOR.":</b> ".EMPTY_SLOT."<br />\n");
...
if ($arrEquip[5][0])
{
    ...
    $smarty -> assign ("Shield", $strWarn."<b>".SHIELD.":</b> ".$arrEquip[5][1]." (+".$arrEquip[5][2].")(+<b>$tarczaspeed</b> szyb) ".$agility1." (".$arrEquip[5][6]."/".$arrEquip[5][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[5][0]."\">".WEAR_OFF."</a>][<a href=\"equip.php?napraw=".$arrEquip[5][0]."\">".A_REPAIR."</a>]<br />\n");
}
    else
{
...
if ($arrEquip[4][0])
{
    ...
    $smarty -> assign ("Legs", $strWarn."<b>".LEGS.":</b> ".$arrEquip[4][1]." (+".$arrEquip[4][2].")(+<b>$nagolespeed</b> szyb) ".$agility2." (".$arrEquip[4][6]."/".$arrEquip[4][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[4][0]."\">".WEAR_OFF."</a>][<a href=\"equip.php?napraw=".$arrEquip[4][0]."\">".A_REPAIR."</a>]<br />\n");
}
    else
{
...
if ($mik -> fields['id'])
{
    ...
    while (!$mik -> EOF)
    {
        ...
        $arrsellpotion[$i] = $mik2 -> fields['illanias']/5 + $mik2 -> fields['illani']/5 +$mik2 -> fields['nutari']/5 + $mik2 -> fields['dynallca']/5;
...
$test = $db -> Execute("SELECT szyb FROM equipment WHERE type='A' AND status='E' AND owner=".$player -> id);
$szyb = $test -> fields['szyb'];
$smarty -> assign(array("szyb" => $szyb));
$test = $db -> Execute("SELECT szyb FROM equipment WHERE type='L' AND status='E' AND owner=".$player -> id);
$nagole = $test -> fields['nagole'];
$smarty -> assign(array("nagole" => $nagole));
$test = $db -> Execute("SELECT szyb FROM equipment WHERE type='S' AND status='E' AND owner=".$player -> id);
$tarcza = $test -> fields['tarcza'];
$smarty -> assign(array("tarcza" => $tarcza));


$sru = $db -> Execute("SELECT zr FROM equipment WHERE type='W' AND status='E' AND owner=".$player -> id);
$fiz = $sru -> fields['fiz'];
$smarty -> assign(array("fiz" => $fiz));
/**
