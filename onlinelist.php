...
if ($player[1])
{
	$sql .= ", sum(`reputation`.`points`), `miejsce`, `page`, `rasa`, `hp`, `klasa`, `level`, `avatar`";
}
$sql2 = " FROM `players`";
...
for ($i = 0, $intMax = count($arrPlayers), $arrPlayers2 = array(); $i < $intMax; $i++)
{
	...
	if ($arrPlayers[$i][9] =='Altara')
		$arrPlayers[$i][9] = ($arrPlayers[$i][8] =='Altara') ? 'Aleara' : 'Coweritana';
	...
	*/
	...
	if ($player[1])
	{
		...
		$arrPlayers2[$i]['klasa'] = $arrPlayers[$i][12];
		$arrPlayers2[$i]['poziom'] = $arrPlayers[$i][13];
		$arrPlayers2[$i]['avek'] = "avatars/".$arrPlayers[$i][14];
	}
}
