...
$aRights = array(
    'ban' => array('Admin', 'Staff', 'Karczmarka', 'Strażnik Miejski'),
    'unban' => array('Admin', 'Staff', 'Karczmarka', 'Strażnik Miejski'),
    'bot' => array('Admin', 'Staff', 'Karczmarka', 'Królowa Potępionych', 'Strażnik Miejski', 'Druid', 'Mistrz Gry'),
    'clear' => array('Admin', 'Karczmarka')
);
...
function send($sMsg, $sSender, $iSender, $sLang, $iOwner = 0)
{
    ...
    $db -> Execute('INSERT INTO chat(sender, text, senderid, ownerid, lang, time, room) VALUES('.$sSender.','.$sMsg.','.$iSender.','.$iOwner.','.$sLang.','.$time.', "piwnica")') or die($db -> ErrorMsg());
}
...
$arrUser = array();
$users = $db -> Execute('SELECT cu.userid, p.user, p.rank, p.tavern_color FROM chat_users cu, players p WHERE cu.room="piwnica" AND cu.time > '.($time - 30).' AND cu.userid = p.id ORDER BY user') or die($db -> ErrorMsg());
while(!$users -> EOF)
{
    $arrUser[] = array(
