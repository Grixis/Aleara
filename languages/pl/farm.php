...
if (!isset($_GET['step']))
{
    define('FARM_INFO', '<p>Przekraczając Bramy Miasta kierujesz się w stronę górskich 
				hal. Z niedowierzeniem wodzisz wzrokiem wzdłuż traktu, który 
				rozciąga się po całej okolicy. Niczym wąż oplata urodzajne 
				masywy, by za moment łagodnie opaść po ich grzbietach ku 
				dolinie. Wiele słyszałeś o tym miejscu, żyzne i dobrze 
				nasłonecznione stoki w Arear ściągały od dawien dawna rzesze 
				plantatorów. To samo tyczyło się dobrze nawodnionych i 
				zacienionych dolin. Bowiem, ze względu na urozmaiconą rzeźbę 
				terenu i różnorodność warunków klimatycznych, możliwa jest tu 
				uprawa wielu odmian roślin. Kiedy tak wędrujesz po okolicy 
				przyglądając się pracy plantatorów, dostrzegasz nieopodal drogi 
				drewnianą chatę. Postanawiasz zasięgnąć informacji z pierwszej 
				ręki. Jednak w momencie, gdy wkraczasz na teren posesji, drzwi 
				gwałtownie się otwierają i dobiega Cię nieprzyjazny głos 
				gospodarza.<br />
				- <i>Czego tu! Jazda mi stąd, pełno tu się takich kręci co tylko 
				czekają, aby coś zwędzić lub zniszczyć. Nie wyglądasz na kogoś, 
				kto się para ciężką robotą. Ręce masz nieurobione!</i><br />
				Zaskoczony reakcją mężczyzny zaczynasz chaotycznie opowiadać o 
				swych planach i celu przybycia, zachwalając raz po raz jego 
				obejście. Po dłuższym czasie starzec uspokoiwszy się nieco, 
				zerka na Ciebie przychylniejszym okiem i tak o to rzecze 
				spokojnym już tonem.<br />
				- <i>Było mówić, że hodować zioła chce. Dobrej ziemi u Nas pod 
				dostatkiem, choćby tam na południowym wzgórzu sporo wolnych 
				parceli. Każdy się dorobi jeśli jeno głową ruszy i silną wolę 
				ma. Kawałek ziemi odsprzedam na początek, dużo nie wezmę. Reszta 
				to już od Ciebie zależy, nie moje zmartwienie. Więc jak, 
				szykować papiery sprzedaży?</i></p>
				<p></p>
				<p>Witaj na farmach. Możesz tutaj hodować zioła z których 
				później wyrabia się mikstury.</p>
');
...
{
    define('HOUSE_INFO', 'Witaj w chatce ogrodnika. Tutaj możesz wysuszyć swoje zioła i otrzymać za to paczki nasion, którymi możesz potem zasiać farmę. Aby otrzymać jedną paczkę nasion potrzebujesz 5 ziół.');
    define('A_DRY', 'Wysusz');
    define('T_DRY', 'aby otrzymać');
    define('T_PACK', 'paczek nasion');
    define('HERB1', 'Zioło Illani');
    define('HERB2', 'Zioło Illanias');
    define('HERB3', 'Zioło Nutari');
    define('HERB4', 'Zioło Dynallca');
...
{
    define('FARM_INFO1', 'Witaj na plantacji w cesarstwie ');
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'upgrade')
    {
        ...
        define('FIELD', 'Pole (wymagany mithril)');
        define('GLASS', 'Szklarnia (wymagana sosna)');
        define('IRRIGATION', 'Nawadnianie (wymagana sosna)');
        define('CREEPER', 'Konstrukcje (wymagana sosna)');
