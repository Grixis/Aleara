...
define('A_NAME', 'Zmień imię postaci');
define('A_PASS', 'Zmień hasło do konta');
define('A_PROFILE', 'Edytuj swój profil');
define('A_EMAIL', 'Edytuj informacje kontaktowe');
define('A_AVATAR', 'Zmień avatar postaci');
...
define('A_CHANGES', 'Sprawdź ostatnie zmiany');
define('A_BUGREPORT', 'Zgłoś błąd w grze');
define('A_BUGTRACK', 'Lista zgłoszonych błędów');
define('A_LINKS', 'Dodaj link do własnego menu');
define('DESCRIPTION', 'Zmień swój opis');
define('RESETFABULARNY', 'Reset fabularny');
define('SIGNATURES', '');
define('ADD', 'Dodaj');
define('EDIT', 'Edytuj');
define('DEL', 'Usun');
define('VIEW_P', 'Pokaz');
define('ADD_A_TAB','Nie masz jeszcze żadnych zakładek');
define('SELECT_A_TAB','Wybierz zakładkę');
...
if (isset($_GET['view']) && $_GET['view'] == 'links')
{
    ...
    define('YOU_CHANGE', 'Zmieniono link w menu nawigacji.');
    define('YOU_ADD', 'Dodano link w menu nawigacji.');
    define('LINK_DELETED', 'Usunięto link w menu nawigacji.');
}
...
if (isset($_GET['view']) && $_GET['view'] == 'bugreport')
{
    ...
    define('BUG_INFO', 'Tutaj możesz zgłosić błÄd w grze. Staraj się opisać dokładnie co się wydarzyło, co robiłeś zanim ten błąd wystąpił oraz jeżeli na ekranie pojawiły się jakieś informacje na ten temat, podaj je. Postaraj się nadać zgłoszeniu odpowiedni tytuł. Zgłoszenia typu \'Błąd\' będą z niewiele mówiącym tytułem nie będą brane pod uwagę! Jeżeli nie wiesz co napisać skorzystaj z poradnika <a href="http://www.chiark.greenend.org.uk/~sgtatham/bugs-pl.html">Jak efektywnie zgłaszać błędy</a>');
}
...
if (isset ($_GET['view']) && $_GET['view'] == 'options')
{
    ...
    define('T_BATTLELOGA', 'Wysyłanie przebiegu walki na pocztę jeżli atakujesz');
    define('T_BATTLELOGD', 'Wysyłanie przebiegu walki na pocztę jeżli jesteś atakowany');
    define('T_GRAPHBAR', 'Wyświetlanie graficznych pasków życia/zdrowia/doświadczenia');
    define('T_OVERLIB', 'Wyświetlanie menu nawigacji w panelach');
...
{
    define('FREEZE_INFO', 'Tutaj możesz zamrozić swoje konto na określony czas. W tym czasie nie będziesz mółgł wejść na nie, ale również nie będziesz mółgł być okradziony czy też zaatakowany. Niestety, nie ma możliwości zamrożenia również Twojej strażnicy. Na zamrożone konto nie przychodzi energia co reset. Maksymalny czas blokady - 21 dni. Blokada rozpoczyna od zaraz.');
    define('HOW_MANY', 'Podaj na ile dni chcesz zablokowaÄ konto');
...
{
	define('IMMU_INFO1', '<b>Immunitet</b> jest specjalną opieką, jaką Władcy otaczają rzemieślników wytwarzających swoje dobra na ziemiach Krainy. Rzemieślnicy, którzy zgodzą się na posiadanie takiej opieki nie mogą być atakowani przez innych mieszkańcółw, a sami mogą walczyć jedynie z potworami. Rezygnacja z ochrony Namiestników niesie ze sobą obniżenie statystyk postaci o 25%. <b>Jako, że obrałeś drogę walki lub złodziejstwa, nie możesz posiadać immunitetu</b>.');
	define('IMMU_INFO2', 'Jeśli nie chcesz, by w Twojej pracy przeszkadzali Ci mieszkańcy pragnący wyzwać Cię na pojedynek, Namiestnicy oferują Ci ochronę. Pod ich opieką żaden mieszkaniec nie napadnie Ciee  sam jednak będziesz  mógł walczyć jedynie z potworami. Zanim się zdecydujesz na ochronę władców pamiętaj, że rezygnacja z niej spowoduje bardzo znaczące osłabienie Twej postaci.');
	define('QUESTION1', 'Czy chcesz wziąść immunitet?');
	define('DISCARD_INFO', 'Posiadasz już immunitet. Teraz możesz go zdjąć, ale wiąże się to z utratą statystyk (-25% aktualnych wartoĹci).');
	define('QUESTION2', 'Czy chcesz zdjąć immunitet?');
	define('YOU_HAVE', 'Posiadasz już immunitet!');
	define('NO_CLASS', 'Musisz najpierw wybrać klasę postaci!<ul><li><a href=account.php>Wróć
	</a></li><li><a href=klasa.php>Wybierz klasę</a></li></ul>');
...
{
    define('RESET_INFO', 'Tutaj możesz zresetować swoją postać. Na Twój adres email zostanie wysłany specjalny link aktywacyjny. Dopiero po kliknięciu na niego Twoja postać zostanie zresetowana.  Czy chcesz zresetować postać');
    if (isset ($_GET['step']) && $_GET['step'] == 'make')
    {
        define('MESSAGE1', 'Dostales tego maila poniwaz chciales zresetowac swoja postac w grze Aleara MMORPG. Jesli jestes pewien, ze chcesz to zrobic - wejdz w ponizszy link. Pozdrawia Administracja Aleary.                   ');
        define('ID', 'ID');
...
if (isset($_GET['view']) && $_GET['view'] == 'avatar')
{
    ...
    define('AVATAR_INFO', 'Tutaj możesz zmienić swoój avatar. <b>Uwaga!</b> Jeżeli już posiadasz avatar, stary zostanie skasowany. Maksymalny rozmiar pliku to <b>100 kB </b>. Avatar możesz załadować tylko z własnego komputera. Musi on mieć rozszerzenie *.<b>jpg</b>, *.<b>jpeg</b>, *.<b>gif</b> lub *.<b>png</b>.');
    define('A_DELETE', 'Skasuj');
...
if (isset($_GET['view']) && $_GET['view'] == 'name')
{
    ...
    {
        define('EMPTY_NAME', 'Podałeś puste imię.');
...
{
    define('PASS_INFO', 'Nie używaj HTML, ani pojedyńczego cudzysłowu. Nie prółbuj go używać, będzie usunięty.');
...
{
	define('PODPIS_TEXT', 'Tutaj możesz zmienić opis swojej postaci. Będzie widoczny w dwółch miejscach: w dymku po najechaniu kursorem na Twoją postać i na liście zalogowanych mieszkańców pod imieniem (operujśc linkiem "Pokaż/ukryj opisy"). Opis może mieć maksymalnie +100 znaków, reszta zostanie ucięta.');
...
{
    define('HEAD_TEXT', '<p>W tym miejscu możesz wybrać styl swojej spersonalizowanej sygnatury, którą możesz wstawić na ulubione forum czy stronę internetową. Jest to bardzo ciekawy sposób na informowanie innych o Twojej przygodzie w Krainie.</p><p>Oto dostępne sygnatury wraz z gotowym kodem do wklejenia:</p>');
}
?>
