<?php /* Smarty version 2.6.16, created on 2012-10-06 01:30:13
         compiled from core.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'cycle', 'core.tpl', 427, false),)), $this); ?>
	<center><IMG SRC="images/locations/chowance.png"></center>
	<img style="margin-left: -8px" width="538px"  src="liniuszka.png"><br><br>
		Dziś postanowiłeś iść na skraj miasta, w zasadzie to nawet opuścić miasto. Wyszedłeś za jego bramy mijając nieco podchmielonych strażników pozdrawiając ich podniesieniem dłoni. Odpowiedzieli szczerbatymi uśmiechami, macając czy nie zostawili w wieży strażniczej swych broni. <br>
		Wyszedłeś beztrosko po za mury i skręciłeś w prawo by iść ledwo widoczną ścieżyną wzdłuż murów. Mijając głazy i zarośla i przebijając się przez tyle co zbudowane pajęczyny powoli zacząłeś dostrzegać cel swojej drogi. Wyszedłeś na polanę na której mieszkańcy trenowali swe chowańce. Pozdrowiłeś machnięciem znajomych bywalców tegoż zakątka oni zajęci machnęli tobie uśmiechając się do Ciebie. Część z nich wystawiała swoje zwierzaki do walk trenigowych. Skierowałeś się w stronę zagrody ze swoim zwierzyńcem by przywitać się z czekającymi na Twoje przybycie Chowańcami.

<?php echo '';  if ($this->_tpl_vars['Corepass'] != 'Y'):  echo '';  echo @COREPASS_INFO;  echo '';  if ($this->_tpl_vars['Gold'] >= '500'):  echo '<br/><br/>';  echo @HAVE_MONEY;  echo '<ul><li><a href="core.php?answer=yes">';  echo @A_YES;  echo '</a></li><li><a href="city.php">';  echo @A_NO;  echo '</a></li></ul>';  else:  echo '<br/><br/>';  echo @NO_MONEY;  echo '';  endif;  echo '';  else:  echo '';  if ($this->_tpl_vars['view'] == 'none'):  echo '<ul><li><a href="core.php?view=my">';  echo @A_MY_CORE;  echo '</a></li><li><a href="core.php?view=arena">';  echo @A_ARENA;  echo '</a></li><li><a href="core.php?view=train">';  echo @A_TRAIN;  echo '</a></li><li><a href="core.php?view=market">';  echo @A_MARKET;  echo '</a></li><li><a href="core.php?view=search">';  echo @A_SEARCH;  echo '</a></li><li><a href="core.php?view=breed">';  echo @A_BREED;  echo '</a></li><li><a href="core.php?view=monuments">';  echo @A_MONUMENTS;  echo '</a></li></ul><br/><a href="city.php">';  echo $this->_tpl_vars['Back'];  echo '</a>';  elseif ($this->_tpl_vars['view'] == 'my'):  echo '';  echo @MY_CORES_INFO;  echo '<table width="90%"><tr><td width="50%" valign="top"><fieldset style="overflow:auto;"><legend><b>';  echo @NORMAL_ARENA;  echo '</b></legend><ul>';  unset($this->_sections['foo']);
