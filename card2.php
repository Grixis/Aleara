...
require_once('languages/pl/card2.php');
...
if ($_GET['action'] == 'class')
{
	...
	if (isset($_GET['select']) && $_GET['select'] != '' && isset ($_GET['step']) && $_GET['step'] == 'mark') 
		...
		$db -> Execute('UPDATE `players` SET `klasa`=\''.$arrClasses[$intKey].'\' WHERE `id`='.$player -> id);
		if ($player -> deity != '')
		{
			error (C_CS);
		}
...
if ($_GET['action'] == 'place')
{
	...
	else
	{
		...
		{
			$smarty -> assign(array('City1' => '<li><a href="card2.php?action=place&amp;select='.strtolower($city1).'">'.$city1.'</a> (To miasto będzie najlepsze dla Ciebie)</li>',
						));
		}
