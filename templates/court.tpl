...
{if $Step2 == "" && $Modify == ""}
    ...
    <tr>
        <td valign="top" width="50%">
            <u><b>{$Tlists}</b></u><ul>
            <li><a href="court.php?list=judges">{$Ajudges}</a></li>
            <li><a href="court.php?list=aldermen">{$Aaldermen}</a></li>
            <li><a href="court.php?list=lawyers">{$Alawyers}</a></li>
            {if $Rank == "Admin" || $Rank == "Sędzia" || $Rank == "Kanclerz Sądu"}
                <li><a href="court.php?step=admin">{$Aadmin}</a></li>
            {/if}
            </ul>
        </td>
        <td valign="top" width="50%">
            <u><b>{$Tinfo}</b></u><ul>
            <li><a href="court.php?step=rules">{$Arules}</a></li>
            <li><a href="court.php?step=cases">{$Acases}</a></li>
            <li><a href="court.php?step=verdicts">{$Averdicts}</a></li>
            </ul>
        </td>
    </tr>
    </table>
    ...
    {if $Step == "rules" || $Step == "cases" || $Step == "verdicts"}
        ...
        {/if}
        {if $Amount > "0"}
            {$Listinfo} {$Itemsinfo} {$Gamename}. {$Listinfo2}<ul>
            {section name="court2" loop=$Rtitle}
                <li><a href="court.php?step={$Step}&amp;step2={$Rid[court2]}">{$Rtitle[court2]}</a></li>
            {/section}
            </ul>
        {/if}
    {/if}
...
{if $Step2 == "comments"}
    ...
    {/if}
    <br /><br />
    {if $Rank == "Admin" || $Rank == "Sędzia" || $Rank == "Kanclerz Sądu" || $Rank == "Staff"}
        <center>
