...
{* Display main menu.*}
    <ul>
    <li><a href="murgrabia.php?view=addreps">Przyznaj punkty reputacji</a></li>
    <li><a href="murgrabia.php?view=donate">Dotuj użytkownika brylantami</a></li>
    <li><a href="murgrabia.php?view=poczta">Roześlij pocztę do wszystkich graczy</a></li>
    <li><a href="murgrabia.php?view=contest">Ogłoś nową turę</a></li>
	<li><a href="murgrabia.php?view=prunereps" onclick="return confirm('Na pewno chcesz wyzerować reputację? Ta czynność jest nieodwracalna!');">Wyzeruj reputację</a></li>
    </ul>
<br><br>
<center>
<fieldset>
<legend>Info dotyczące bierzącej tury</legend>
{$Turn}
</fieldset>
{/if}

{if $View == "addreps"}
<form method="post" action="murgrabia.php?view=addreps">
    ...
    <table width="100%">
        {section name=staff2 loop=$Ttitle}
            <tr>
                ...
                <td><a href="murgrabia.php?view=addtext&amp;action=modify&amp;text={$Tid[staff2]}">{$Amodify}</a></td>
                <td><a href="murgrabia.php?view=addtext&amp;action=add&amp;text={$Tid[staff2]}">{$Aadd}</a></td>
                <td><a href="murgrabia.php?view=addtext&amp;action=delete&amp;text={$Tid[staff2]}">{$Adelete}</a></td>
            </tr>
        {/section}
    </table>

    {if $Action == "modify"}
        <form method="post" action="murgrabia.php?view=addtext&amp;action=modify&amp;text={$Tid}">
...
{if $smarty.get.view == "bugreport"}
    {if $smarty.get.step != ""}
        ...
        <form method="post" action="murgrabia.php?view=bugreport&amp;step={$smarty.get.step}"  onsubmit="if (this.bugcomment.value=='') {literal}{{/literal} return confirm('{$smarty.const.EMPTY_COMMENT}') } else {literal}{{/literal} return true; }">
    ...
    {else}
        <table align="center" width="100%">
			...
			<tr {if $Bugs[k][5] != "0"}class="bugrepair"{/if}>
				<td width="5%" align="center"><a {if $Bugs[k][5] != "0"}class="bugrepair" {/if}href="murgrabia.php?view=bugreport&amp;step={$Bugs[k][0]}">{$Bugs[k][0]}</td>
...
{/if}
{if $View == "pary"}
    <form method="post" action="murgrabia.php?view=pary&step=add">
...
{if $View == "banmail"}
    ...
    {section name=banmail loop=$List1}
        ID {$List1[banmail]}<br />
    {/section}
    <form method="post" action="murgrabia.php?view=banmail&amp;step=mail">
    <select name="mail">
...
{if $View == "playerquest"}
    <form method="post" action="murgrabia.php?view=playerquest&amp;step=add">
...
{if $View == "changelog"}
    {$Changeinfo}<br /><br />
    <form method="post" action="murgrabia.php?view=changelog&amp;step=add">
...
{if $View == "logs"}
    ...
    <form method="post" action="murgrabia.php?view=logs&amp;step=clear">
...
{if $View == "meta"}
    {$Metainfo}
    <form method="post" action="murgrabia.php?view=meta&amp;step=modify">
...
{if $View == "forums"}
    ...
    {section name=forum loop=$Catname}
        <a href="murgrabia.php?view=forums&amp;id={$Catid[forum]}">{$Catname[forum]}</a><br />
    {/section}<br /><br />
    <form method=post action="murgrabia.php?view=forums&amp;id={$Catid2}&amp;step=add">
...
{if $View == "mill"}
    <form method="post" action="murgrabia.php?view=mill&amp;step=mill">
...
{if $View == "donator"}
    {if $Step == ""}
        {$Donatorinfo}<br />
        <form method="post" action="murgrabia.php?view=donator&amp;step=add">
...
{if $View == "clearf"}
    {if $Step == ""}
    {$Fquestion}<br />
    - <a href="murgrabia.php?view=clearf&amp;step=Y">{$Ayes}</a><br />
    {/if}
{/if}

{if $View == "monster2"}
    {if $Step == ""}
        <form method="post" action="murgrabia.php?view=monster2&amp;step=next">
    ...
    {/if}
    {if $Step == "next"}
        <form method="post" action="murgrabia.php?view=monster2&amp;step=monster">
...
{if $View == "jailbreak"}
    {if $Step == ""}
        <form method="post" action="murgrabia.php?view=jailbreak&amp;step=next">
...
{if $View == "poll"}
    {if $Step == ""}
        <form method="post" action="murgrabia.php?view=poll&amp;step=second">
    ...
    {if $Step == "second"}
        ...
        <form method="post" action="murgrabia.php?view=poll&amp;step=add">
...
{if $View == "censorship"}
    <form method="post" action="murgrabia.php?view=censorship&amp;step=modify">
        <select name="action">
...
{if $View == "register" || $View == "close"}
    <form method="post" action="murgrabia.php?view={$View}&amp;step=close">
    <select name="close">
...
{if $View == "ban"}
    {$Banlist}<br />
    <form method="post" action="murgrabia.php?view=ban&amp;step=modify">
...
{if $View == "mail"}
    <form method="post" action="murgrabia.php?view=mail&amp;step=send">
    {$Mailinfo}<br />
...
{if $View == "bridge"}
    <form method="post" action="murgrabia.php?view=bridge&amp;step=add">
...
{if $View == "jail"}
    {if isset($Confirm)}
        {$Fquestion}<br />
        <form method="post" action="murgrabia.php?view=jail&amp;step=add&amp;confirm=Y">
    ...
    {else}
        <form method="post" action="murgrabia.php?view=jail&amp;step=add">
...
{if $View == "del"}
    <form method="post" action="murgrabia.php?view=del&amp;step=del">
    {$Deleteid}<input type="text" name="did" />.<input type="submit" value="{$Adeletepl}" />
    </form>
{/if}

{if $View == "add"}
    <form method="post" action="murgrabia.php?view=add&amp;step=add">
    ...
    <option value="Strażnik Miejski">{$Rmurgrabia}</option>
...
{if $View == "equipment"}
    <form method="post" action="murgrabia.php?view=equipment&amp;step=add">
...
{if $View == "donate"}
    <form method="post" action="murgrabia.php?view=donate&amp;step=donated">
...
{if $View == "takeaway"}
    {$Takeinfo}<br />
    <form method="post" action="murgrabia.php?view=takeaway&amp;step=takenaway">
    <table>
        <tr>
...
{if $View == "monster"}
    <form method="post" action="murgrabia.php?view=monster&amp;step=monster">
...
{if $View == "kowal"}
    <form method="post" action="murgrabia.php?view=kowal&amp;step=kowal">
    {$Sname}: <input type="text" name="nazwa" /> <br />
    {$Scost}: <input type="text" name="cena" /> <br />
    {$Samount}: <input type="text" name="amount" /><br />
    {$Slevel}: <input type="text" name="poziom" /> <br />
...
{if $View == "poczta"}
    <table>
    <form method="post" action="murgrabia.php?view=poczta&amp;step=send">
...
{if $View == "czat"}
    ...
    {section name=player loop=$List1}
        ID {$List1[player]}<br />
    {/section}
    <form method="post" action="murgrabia.php?view=czat&amp;step=czat">
    <select name="czat">
        <option value="blok">{$Ablock}</option>
        <option value="odblok">{$Aunblock}</option>
    </select>
...
{if $View == "czary"}
    <form method="post" action="murgrabia.php?view=czary&amp;step=add">
...
{/if}
{if $View == "contest"}
Rozpocznij nową turę zawodów. Wprowadź daty rozpoczęcia i zakończenia tury:
<form method="post" action="murgrabia.php?view=contest&amp;action=ok">
<center>
<fieldset width="50%">
...
<input type="text" name="syear" maxlength="4" size="4" />
</fieldset>
<fieldset width="50%">
...
<input type="text" name="eyear" maxlength="4" size="4" />
</fieldset>
...
{if $View != ""}
    <br />(<a href="murgrabia.php">{$Aback}</a>)
{/if}
