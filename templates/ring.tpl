...
{if $Action == "" && $battle == ""}
    ...
    <li><a href="ring.php?action=levellist">Wejdź na Arenę Gladiatorów</a></li>
...
{if $Action == "showalive"}
    ...
    {/section}
    </table><br />{$Orback} <a href="ring.php">{$Bback}</a>.
{/if}

{if $Action == "levellist"}
    <form method="post" action="ring.php?action=levellist&amp;step=go">
    ...
    <table width="100%">
        ...
        {section name=player loop=$Enemyid}
            ...
            <td>{$Enemyrank[player]}</td>
            <td>{$Enemytribe[player]}</td>
            <td>- <A href="ring.php?battle={$Enemyid[player]}">{$Aattack}</a></td>
            </tr>
        {/section}
        </table>
    {/if}
...
{if $Action == "monster"}
    ...
    {else}
        {if !$Fight && !$Fight1}
            ...
            <br /><br />
            {if $Dalej > 0}
                <form method="post" action="ring.php?action=monster&amp;fight={$Id}">
            ...
            {/if}
            {if $Next > 0}
                <form method="post" action="ring.php?action=monster&amp;fight1={$Id}">
                <input type="submit" value="{$Abattle2}" /> {$Witha} <input type="text" size="5" name="razy" value="1" /> {$Name}{$Nend}
