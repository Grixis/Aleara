...
$test = $db -> Execute("SELECT helmspeed FROM equipment WHERE type='H' AND status='E' AND owner=".$player -> id);
$helmspeed = $test -> fields['helmspeed'];
...
$test = $db -> Execute("SELECT nagolespeed FROM equipment WHERE type='H' AND status='E' AND owner=".$player -> id);
$nagolespeed = $test -> fields['nagolespeed'];
...
if (isset($_GET['napraw']))
{
    ...
    else
    {
        ...
        if (!$test -> fields['id'])
        {
            $db -> Execute(RepairEquip($rzecz->fields['id']));
        }
            else
        {
            $db -> Execute("UPDATE `equipment` SET `amount`=`amount`+1 WHERE `id`=".$test -> fields['id']);
