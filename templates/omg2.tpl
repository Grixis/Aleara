...
{if $View == "" && $Read == "" && $Write == "" && $Delete == "" && $Send == "" && $Step == "" && $Block == ""}
    ...
    <ul{if $Graphstyle == "Y"} class="sword"{/if}>
        <li> <a href="omg2.php?view=inbox">Wszystkie Listy</a></li>
        <li> <a href="omg2.php?view=zapis">Zapisane Listy</a></li>
    ...
    {if $Senderid}
        ...
        <table width="100%">
            ...
            <tr>
                ...
                <td>{$Senderid[mail]}</td>
                <td>{$Subject[mail]}</td>
                <td>- <a href="omg2.php?read={$Mailid[mail]}&amp;option=c">Czytaj</a><blink> !!</blink><br />
...
{if $View == "inbox"}
    <form method="post" action="omg2.php?step=mail&amp;box=I">
    <table width="100%">
    <tr>
        <td width="5%"></td>
    ...
    <tr>
        ...
        <td>{$Senderid[mail]}</td>
        <td>{$Subject[mail]}</td>
        <td>- <a href="omg2.php?read={$Mailid[mail]}&amp;option=c">Czytaj</a>{if $Mread[mail] == "Y"} <blink>!!</blink> {/if}<br />
    ...
    <form method="post" action="omg2.php?step=deleteold&amp;box=I">
    ...
    <br />[<a href="omg2.php?view=zapis">{$smarty.const.A_SAVED}</a>]
    [<a href="omg2.php?view=inbox&amp;step=clear">{$smarty.const.A_CLEAR}</a>]
    [<a href="omg2.php?view=write">{$smarty.const.A_WRITE}</a>]
{/if}
...
{if $View == "zapis"}
    ...
    <tr>
        ...
        <td>{$Subject[mail1]}</td>
        <td>- <a href="omg2.php?read={$Mailid[mail1]}&amp;option=c">Czytaj</a><br />
    ...
    <form method="post" action="omg2.php?step=deleteold&amp;box=W">
        <input type="submit" value="{$smarty.const.A_DELETE_OLD}" /> <select name="oldtime">
            <option value="7">{$smarty.const.A_WEEK}</option>
            <option value="14">{$smarty.const.A_2WEEK}</option>
            <option value="30">{$smarty.const.A_MONTH}</option>
    ...
    <br />[<a href="omg2.php?view=inbox">{$smarty.const.A_INBOX}</a>]
    [<a href="omg2.php?view=zapis&amp;step=clear">{$smarty.const.A_CLEAR}</a>]
    [<a href="omg2.php?view=write">{$smarty.const.A_WRITE}</a>]
{/if}

{if $View == "send"}
    <form method="post" action="omg2.php?step=mail&amp;box=S">
    <table width="100%">
    <tr>
        <td width="5%"></td>
    ...
    {section name=mail2 loop=$Send1}
        ...
        <td>{$Subject[mail2]}</td>
        <td>- <a href="omg2.php?read={$Mailid[mail2]}&amp;option=u">{$smarty.const.M_READ}</a></td>
        </tr>
    {/section}
    </table><br />
    <input type="submit" value="{$smarty.const.A_DELETE_S}" name="delete" /> <input type="submit" name="write" value="{$smarty.const.A_SAVE_S}" /><br />
    </form>
    <form method="post" action="omg2.php?step=deleteold&amp;box=S">
        <input type="submit" value="{$smarty.const.A_DELETE_OLD}" /> <select name="oldtime">
            <option value="7">{$smarty.const.A_WEEK}</option>
            <option value="14">{$smarty.const.A_2WEEK}</option>
            <option value="30">{$smarty.const.A_MONTH}</option>
    ...
    <br />[<a href="omg2.php">{$smarty.const.A_BACK}</a>]
    [<a href="omg2.php?view=send&amp;step=clear">{$smarty.const.A_CLEAR}</a>]
    [<a href="omg2.php?view=write">{$smarty.const.A_WRITE}</a>]
{/if}

{if $View == "write"}
    [<a href="omg2.php?view=inbox">{$smarty.const.A_INBOX}</a>]<br /><br />
    <table>
    <form method="post" action="omg2.php?view=write&amp;step=send">
...
{if $Read != ""}
    ...
    [<a href="omg2.php?zapisz={$Mailid[mailer]}">{$smarty.const.A_SAVE}</a>]
    [<a href="omg2.php?kasuj={$Mailid[mailer]}">{$smarty.const.A_DELETE}</a>]
    [<a href="omg2.php?view=write">{$smarty.const.A_WRITE}</a>]
    
    [<a href="omg2.php?send={$Mailid[mailer]}">{$smarty.const.A_SEND}</a>]
    [<a href="omg2.php?block={$Senderid[mailer]}">{$smarty.const.A_BLOCK}</a>]
	</td>
	</tr>
	</table><center><br/><br/>
...
{if $View == "blocks"}
    {if $Blockid[0] != ""}
        <form method="post" action="omg2.php?view=blocks&amp;step=unblock">
            <table>
                <tr>
    ...
    {/if}
    <br /><br />(<a href="omg2.php">{$smarty.const.A_BACK}</a>)
{/if}
{/strip}
