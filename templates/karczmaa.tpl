...
<a href="karczmaa.php?view=innarchive&amp;room=izba">Klimatyczna</a><br/>
<a href="karczmaa.php?view=innarchive&amp;room=piwnica">Bez Klimatu</a><br/>
</fieldset>
<fieldset>
...
{if $View == "czat"}
    ...
    {section name=player loop=$List1}
        ID {$List1[player]}<br />
    {/section}
    <form method="post" action="karczmaa.php?view=czat&amp;step=czat">
    <select name="czat">
...
{/if}
{if $View == "query"}
<form method="post" action="karczmaa.php?view=query&amp;step=make">
...
{/if}
{if $View != ""}
    <br />(<a href="karczmaa.php">{$Aback}</a>)
{/if}
{/strip}
