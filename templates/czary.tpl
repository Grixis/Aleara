...
{if $Cast != ""}
    ...
    {section name=spell4 loop=$Itemname}
        <option value="{$Itemid[spell4]}">{$Itemname[spell4]}(+{$Itempower[spell4]}, {$Itemszyb[spell4]} szybkości, {$Itemzr[spell4]} zręczności, {$Itemwt[spell4]}/{$Itemmaxwt[spell4]} ({$Iamount}: {$Itemamount[spell4]})</option>
    {/section}
    </select>
