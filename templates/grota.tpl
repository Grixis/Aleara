...
{if $Corepass != "Y"}
	...
	{if $Gold >= "500"}
		...
		<ul>
		<li><a href="grota.php?answer=yes">{$smarty.const.A_YES}</a></li>
		<li><a href="diabolium.php">{$smarty.const.A_NO}</a></li>
		</ul><br><br>
	{else}
	{$smarty.const.NO_MONEY}
	{/if}
{else}
	{if $view == "none"}
	<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
	<img style="margin-left: -8px" width="538px"  src="liniuszka.png"><br><br>
		Przechadzając się przez Diabolium napotkałeś na swojej drodze kolda który zaczął Cię nakłaniać do zakupu własnego wojownika który będzie Ci służyć. Kold zaprowadził Cię do jamy wojowników płomieni. Widzisz jak inni trenują swoich wojowników. Zaczyna Ci się to podobać. Zauważasz także, że jest tutaj miejsce gdzie odbywają się walki wojowników z ciekawymi nagrodami. Przechodzisz obok jednego z wojownika. Ten niemal rzucił się na Ciebie. Chyba mu się nie spodobałeś. Kold widząc to zaczyna Ci opowiadać o tych goblinowatych stworach. Dowiadujesz się, że stworzył je sam demon Diaburion.
		<ul>
			<li><a href="grota.php?view=my">Moi wojownicy płomieni</a></li>
			<li><a href="grota.php?view=arena&arena=Z">Walki wojowników</a></li>
			<li><a href="grota.php?view=train">Trenuj wojownika</a></li>
			<li><a href="kupwoja.php">Złap swojego wojownika</a></li>		
			<li><a href="grota.php?view=monuments">Najlepsi wojownicy</a></li>
		</ul>
		<br/>
		<a href="diabolium.php">{$Back}</a>
	{elseif $view == "my"}
	<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
		...
		<fieldset style="overflow:auto;">
			<legend><b>Twoi wojownicy</b></legend>
			<ul>
			{section name=foo loop=$Normalnamelist}
				<li>
					<a href="grota.php?view=my&amp;id={$Normalidlist[foo]}">{$Normalnamelist[foo]}</a> [{$Normalstatuslist[foo]}]
				</li>
			{/section}
			</ul>
		</fieldset>
		...
		<br/><br/>
		<a href="grota.php">{$smarty.const.BACK}</a>
	{elseif $view == "display"}
		<center>
		...
		<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
			<br/><form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=changename">
		...
		{elseif $action == "heal" && $Healinfo != ""}
			...
			{$Healinfo} <br/>
			{if $Nomoney == ""}
			<form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=heal">
		...
		{elseif $action == "resurrect" && $Resinfo != ""}
			...
			{$Resinfo} <br/>
			{if $Nomoney == ""}
			<form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=resurrect">
		...
		{elseif $action == "free"}
			...
			<form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=free">
		...
		{elseif $action == "pass"}
			...
			<br/><form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=pass">
			<input type="text" name="corepass" size="5"/>
		...
		{elseif $action == "sell" && $Sellinfo != ""}
			<br/>
			{$Sellinfo}
			<br/><form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=sell">
		...
		{elseif $action == "activate" && $Activateinfo != ""}
			...
			{if $Nomoney == ""}
			<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
				<form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=activate">
		...
		{elseif $action == "deactivate" && $Deactivateinfo != ""}
			...
			{if $Nomoney == ""}
			<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
				<form method="post" action="grota.php?view=my&amp;id={$Coreid}&amp;action=deactivate">
		...
		{/if}
		<table width="80%">
			<tr><td width="30%">{$smarty.const.C_NAME}:</td><td width="70%">{$Name} (<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=changename">{$smarty.const.CHANGE_NAME}</a>)</td></tr>
			
			
			<tr><td>{$smarty.const.C_STATUS}:</td><td>{$Status}</td></tr>
			<tr><td>{$smarty.const.C_HP}:</td><td>{$Hp} / {$Maxhp}
			{if $Hp > 0 && $Maxhp > $Hp}
				&nbsp;(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=heal">{$smarty.const.A_HEAL}</a>)
			{elseif $Hp <= 0}
				&nbsp;(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=resurrect">{$smarty.const.A_RESURRECT}</a>)
			{/if}
			</td></tr>
			...
			<tr><td>(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=free">{$smarty.const.A_FREE}</a>)</td>
			<td>
				{if $Statuscode == 'N'}
				(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=pass">{$smarty.const.A_PASS}</a>)
				{/if}
			</td></tr>
			<tr>
				<td>
				{if $Statuscode == 'N' && $Hp > 0}
					(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=sell">{$smarty.const.A_SELL}</a>)
				{/if}
				</td>
				<td>
				{if $Statuscode == 'S'}
					(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=unsell">{$smarty.const.A_UNSELL}</a>)
				{/if}
				</td>
			</tr>
			<tr>
				<td>
				{if $Statuscode == 'N' && $Hp > 0}
					(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=activate">{$smarty.const.A_ACTIVATE}</a>)
				{/if}
				</td>
				<td>
				{if $Statuscode == 'A'}
					(<a href="grota.php?view=my&amp;id={$Coreid}&amp;action=deactivate">{$smarty.const.A_DEACTIVATE}</a>)
				{/if}
				</td>
			</tr>
		</table>
	...
	{if $action == "search"}
	<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
		Sprawdź czy podszedł do Ciebie jakiś wojownik płomieni.
	{/if}
	...
	<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
		...
		{if $SellNamelist != ""}
			...
			<form method="post" action="grota.php?view=my&amp;action=sell">
			<select name="id">
		...
		{section name=foo loop=$Namelist}
			<li>
			<a href="grota.php?view=market&amp;buy={$Idlist[foo]}">{$smarty.const.A_BUY} - {$Prizelist[foo]} {$smarty.const.GOLD_PIECES}</a>
	...
	{elseif $view == "arena"}
		...
		<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
			{if $Fight == ""}
				...
				<br/><br/>
				<ul>
				<li><a href="grota.php?view=arena&amp;arena=Z">{$smarty.const.ARENA_Z}</a></li>
				<li><a href="grota.php?view=arena&amp;arena=M">{$smarty.const.ARENA_M}</a></li>
				</ul>
			{else}
				<b>{$Fight}</b><br/><br/>
				{$Log}<br/><br/>
		...
		{else}
			...
			{/if}
			<br/><br/>
			<form method="post" action="grota.php?view=arena&amp;action=fight">
	...
	<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
		...
		{if $Namelist != ""}
			...
			<form method="post" action="grota.php?view=train">
			<select name="id">
			{section name=foo loop=$Namelist}
				<option value="{$Idlist[foo]}">
					{$Namelist[foo]} :: {$Attacklist[foo]}, {$Defencelist[foo]}, {$Speedlist[foo]} :: {$Hplist[foo]}
	...
	{elseif $view == "breed"}
		...
		{if $Result == "" && $Choosen == ""}
			<form method="post" action="grota.php?view=breed">
	...
	<center><IMG SRC="images/locations/wojownikplomieni.png"></center>
		...
		<fieldset style="width: 70%; margin:auto;">
			<legend align="center" class="monlegend">Najlepsi wojownicy płomienii:</legend>
			<table width="100%">
				<tr>
		...
		<a href="grota.php">{$smarty.const.BACK}</a></br>
	{/if}
	{/if}
{/strip}
