...
require_once("includes/head123456789.php");
...
if (isset($_GET['step']) && $_GET['step'] == 'redaction')
{
    ...
    */
    if (isset($_GET['step3']) && ($_GET['step3'] == 'edit' || $_GET['step3'] == 'R'))
    {
        ...
        if (isset($_POST['show']))
        {
            $arrType = array('M', 'N', 'O', 'R', 'K', 'C', 'S', 'H', 'I', 'A', 'P', 'X', 'Y', 'Z');
        ...
        if (isset($_POST['sendmail']))
        {
            ...
            if (empty($_POST['mtitle']) or empty($_POST['mbody']))
            {
                error(EMPTY_FIELDS);
            }
            $_POST['mbody'] = nl2br($_POST['mbody']);
                ...
                else
            {
                ...
                $objPaperid -> Close();
                $db -> Execute("INSERT INTO `newspaper` (`paper_id`, `title`, `body`, `author`, `lang`, `added`, `type`) VALUES(".$intPaperid.", ".$strTitle.", ".$strBody.", '".$strAuthor."', '".$player -> lang."', 'M', '".$_POST['mail']."')");
            }
...
if (isset($_GET['step']) && $_GET['step'] == 'mail')
{
    ...
    if (isset($_GET['step3']) && $_GET['step3'] == 'add')
    {
        ...
        $_POST['mbody'] = nl2br($_POST['mbody']);
        require_once('includes/bbcode.php');
        $_POST['mbody'] = mysql_real_escape_string($_POST['mbody']);
        $_POST['mtitle'] = mysql_real_escape_string($_POST['mtitle']);
        if (isset($_POST['show']))
        {
            $strMail = T_TITLE." ".$_POST['mtitle']."<br />".T_BODY." <br />".$_POST['mbody'];
            $_POST['mbody'] = mysql_real_escape_string($_POST['mbody']);
