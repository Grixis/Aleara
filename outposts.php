...
if (isset ($_GET['view']) && $_GET['view'] == 'battle')
{
	if (isset($_POST['oid']) && strictInt($_POST['oid']) && isset($_POST['amount']) && strictInt($_POST['amount']))
	{
		...
		if (!$enemyOut -> id)
			error (NO_OUT);
		if ($enemyOut -> size < round($out ->size / 2)-1)
			error ("Nie możesz zaatakować tej strażnicy");
		$arrEnemy = $db -> getRow('SELECT `user`, `leadership`, `tribe` FROM `players` WHERE `id`='.$enemyOut -> ownerId);

		if ($_POST['amount'] > 3 || $enemyOut -> attacks > 2)
			error(TOO_MUCH_A);
		if (($arrEnemy[2] == $player->tribe) && $player->tribe != '')
		{
			//error("Nie możesz atakować strażnic własnego klanu");
		}
...
/// Get info about selected veteran for edition.
		{
			...
			for ($i = 2; $i < 17; $i += 2)
				if ($arrVeteran[$i] != '')
				{
					$arrVeteran[$i] .= ' (+'.$arrVeteran[$i+1].')';  // add "power" to item's name
