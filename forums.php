...
if (isset ($_GET['view']) && $_GET['view'] == 'categories')
{
    if (isset($_GET['sweep']) && ereg('^[1-9][0-9]*$', $_GET['sweep']))
    {
        if (!isset($_GET['step']))
        {
            ...
            if ($player-> rank =='Admin' || $player -> rank == 'Staff' || $player -> rank == 'Mistrz Gry')
        {
            $objTest = $db -> Execute('SELECT `id`,`perm_write`  FROM `categories` WHERE `id`='.$_GET['sweep']);
            if (strpos($objTest -> fields['perm_write'], 'All') === false && strpos($objTest -> fields['perm_write'], 'Staff') === false && $player -> rank != 'Admin' && $player -> rank != 'Mistrz Gry')
            {
                error(NO_PERM);
            }
                else
...
if (isset($_GET['topic']))
{
	...
	$temp=$db->Execute("SELECT avatar, pktfabularne FROM players WHERE id=".$topicinfo->fields['gracz']);
	$tpkt=$temp->fields['pktfabularne'];
	$plik='avatars/'.$temp->fields['avatar'];
	if (is_file($plik))
    {
       	require_once('includes/avatars.php');
    	$arrImage = scaleavatar($plik);
   		$tavek="<img src=\"".$plik."\" width=\"".$arrImage[0]."\" height=\"".$arrImage[1]."\" />";
	}
    /**
     * Check for permissions
     */
    ...
    $smarty -> assign ('Action', ($player -> rank == 'Admin' || $player -> rank == 'Staff'  || $player -> rank == 'Mistrz Gry' || $player -> id == $topicinfo->fields['gracz']) ? ' (<a href="forums.php?kasuj1='.$topicinfo -> fields['id'].'">'.A_DELETE.'</a>)(<a href="forums.php?editto='.$topicinfo->fields['id'].'">Edytuj</a>)' : '' );
    ...
    while (!$reply -> EOF)
    {
		...
		$temp=$db->Execute("SELECT avatar, pktfabularne FROM players WHERE id=".$arrplayerid[$i]);
		$plik='avatars/'.$temp->fields['avatar'];
		$arrPkt[$i]=$temp->fields['pktfabularne'];
		if (is_file($plik))
        {
       	require_once('includes/avatars.php');
    	$arrImage = scaleavatar($plik);
   		$arrAvat[$i]="<img src=\"".$plik."\" width=\"".$arrImage[0]."\" height=\"".$arrImage[1]."\" />";
		}
        $arraction[$i] = ($player -> rank == 'Admin' || $player -> rank == 'Staff' || $player -> rank == 'Namiestnik' || $player -> rank == 'Hrabia' || $player->id ==$arrplayerid[$i] || $player->id == $topicinfo->fields['gracz']) ? '(<a href="forums.php?kasuj='.$reply -> fields['id'].'">'.A_DELETE.'</a>) (<a href="forums.php?editpo='.$reply->fields['id'].'">Edytuj</a>)' : '';
...
if (isset($_GET['reply']))
{
	...
	$db -> Execute('UPDATE players SET pktfabularne=pktfabularne+1 WHERE id='.$player->id);
...
if (isset($_GET['kasuj']))
{
	...
	$temp=$db -> Execute('SELECT * FROM replies WHERE id='.$_GET['kasuj']);
	$temp2=$db -> Execute('SELECT * FROM topics WHERE id='.$temp->fields['topic_id']);
	
	if ($player -> rank != 'Admin' && $player -> rank != 'Staff' && $player -> rank != 'Namiestnik' && $player -> rank != 'Hrabia' && $player->id != $temp->fields['gracz'] && $player->id != $temp2->fields['gracz'])
    {
        error(ERROR);
    }
	...
	$db -> Execute('UPDATE players SET pktfabularne=pktfabularne-1 WHERE id='.$temp->fields['gracz']);
...
{
	if (!ereg("^[1-9][0-9]*$", $_GET['editto']))
    {
        error(ERROR);
    }
	

	$topicid= $db -> Execute("SELECT * FROM topics WHERE id=".$_GET['editto']);
	$smarty -> assign("Tbody",$topicid->fields['body']);
	$smarty -> assign("Etopic",$_GET['editto']);
	if (isset($_GET['edited']))
	{
	if ($player->id != $topicid -> fields['gracz'] )
	{
   		error(ERROR);
 	}
 	$tid= $topicid->fields['id'];
 	$db -> Execute("UPDATE topics SET body='".$_POST['edycja']."' WHERE id=".$_GET['editto']);
 	error("Post zaktualizowany! <a href='forums.php?topic=".$_GET['editto']."'>".A_BACK."</a>");
 	}
}
/**
* Edit post
...
{
	if (!ereg("^[1-9][0-9]*$", $_GET['editpo']))
    {
        error(ERROR);
    }
	
	$topicid= $db -> Execute("SELECT * FROM replies WHERE id=".$_GET['editpo']);
	$topicid2= $db -> Execute("SELECT * FROM topics WHERE id=".$topicid->fields['topic_id']);
	$smarty -> assign("Tbody",$topicid->fields['body']);
	$smarty -> assign("Epost",$_GET['editpo']);
	if (isset($_GET['edited']))
	{
	if ($player->id != $topicid -> fields['gracz'] and $player-> id != $topicid2 -> fields['gracz'])
	{
   		error(ERROR);
 	}
 	$tid= $topicid->fields['id'];
 	$db -> Execute("UPDATE replies SET body='".$_POST['edycja']."' WHERE id=".$_GET['editpo']);
 	error("Post zaktualizowany! <a href='forums.php?topic=".$topicid->fields['topic_id']."'>".A_BACK."</a>");
 	}
...
$smarty -> assign(array('View' => $_GET['view'],
    'Topics' => $_GET['topics'],
	'Edittopic' => $_GET['editto'],
	'Editpost' => $_GET['editpo'],
    'Topic' => $_GET['topic'],
    'Action3' => $_GET['action'],
    'Rank' => $player -> rank));
