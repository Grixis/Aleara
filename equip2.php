...
$title = "Ekwipunek";
if (!isset($_GET['show'])) require_once("includes/head.php");
else require_once("includes/minihead.php");
...
if (isset ($_GET['item']))
{
	...
	{
		error ("Nie możesz wyslać do samego siebie.");
	}
	...
	{
		error ("Nie masz tylu sztuk ".$item -> fields['name']);
	}
	...
	switch($item -> fields['type'])
	{
		case 'A':   // Pieces of armor: defense, agility and durability.
			...
			$strAttributes.= 'obrona +'.$item -> fields['power'].', zrecznosc '.$intAgi.'%, wytrzymalosc '.$item -> fields['wt'].'/'.$item -> fields['maxwt'];
			break;
		case 'B':   // Bows: speed and durability.
			$strAttributes.= 'szybkosc +'.$item -> fields['szyb'].', wytrzymalosc '.$item -> fields['wt'].'/'.$item -> fields['maxwt'];
			break;
		case 'H':   // Helmets,
		case 'P':   // plate legs,
		case 'S':   // and shiels: defense and durability.
			$strAttributes.= 'obrona +'.$item -> fields['power'].', wytrzymalosc '.$item -> fields['wt'].'/'.$item -> fields['maxwt'];
			break;
		case 'R':   // Arrows: attack.
			$strAttributes.= 'obrazenia +'.$item -> fields['power'];
			break;
		case 'W':   // Melee weapons: attack and durability.
			$strAttributes.= 'obrazenia +'.$item -> fields['power'].', wytrzymalosc '.$item -> fields['wt'].'/'.$item -> fields['maxwt'];
			break;

		case 'C':   // Mage robe: percent bonus to mana.
			$strAttributes.= 'mana +'.$item -> fields['power'].'%';
			break;
		case 'T':   // Mage wand: percent bonus to spell strength.
			$strAttributes.= 'sila czarow +'.$item -> fields['power'].'%';
			break;
	...
	$db -> Execute("INSERT INTO log (`owner`, `log`, `czas`) VALUES(".$_POST['pid'].",'Gracz <b><a href=view.php?view=".$player -> id.">".$player ->user."</a></b>ID:<b>".$player -> id."</b>, przesyla tobie ".$_POST['amount']." sztuk ".$item -> fields['name']."</b> ".$strAttributes."', ".$strDate.")");
	$db -> Execute("INSERT INTO log (`owner`, `log`, `czas`) VALUES(".$player -> id.",'Wyslales graczowi  <b><a href=view.php?view=".$_POST['pid'].">".$strPlayerName."</a></b> ID<b> ".$_POST['pid'].'</b>, '.$_POST['amount']." sztuk ".$item -> fields['name']."</b> ".$strAttributes."', ".$strDate.")");
...
if (isset($_GET['sell_potion']))
{
    ...
    $smarty -> assign ("Action", YOU_SELL." ".$sell -> fields['name']." ".FOR_A." ".$cost." ".GOLD_COINS.". (<A href=\"equip2.php\">".REFRESH."</a>)");
    $sell -> Close();
}
...
if (isset($_GET['napraw']))
{
    ...
    {
        $smarty -> assign ("Action", "Ta rzecz nie wymaga naprawy.");
    }
    if (!$rzecz -> fields['id'])
    {
        $smarty -> assign ("Action", "Nie ma takiego przedmiotu");
    }
...
*/
    ...
    {
        if (!ereg("^[1-9][0-9]*$", $_GET['poisonid']))
		{
		error ("error linia 709");
		}
		$poison = $db -> Execute("SELECT id, power, amount, name FROM potions WHERE id=".$_GET['poisonid']." AND type='P' AND owner=".$player -> id);
		if (!$poison -> fields['id'])
		{
        error (NO_POTION);
		}
		if (!isset($_GET['poison']) || !ereg("^[1-9][0-9]*$", $_GET['poison']))
        {
            error ("error linia 718");
        }
        $item = $db -> Execute("SELECT * FROM equipment WHERE id=".$_GET['poison']);
        if (!$item -> fields['id'])
        {
            error (NO_ITEMS);
        }
        ...
        if ($intRoll > 50)
        {
            ...
            else
            {
                if ($item -> fields['wt'] <= 100)
                {
                    ...
                    if (!$test -> fields['id'] )
                    {
                        $db -> Execute("INSERT INTO `equipment` (`owner`, `name`, `power`, `type`, `cost`, `zr`, `wt`, `minlev`, `maxwt`, `amount`, `magic`, `poison`, `szyb`, `ptype`, `repair`, wzmocnienie, ulepszeniee) VALUES(".$player -> id.",'".$name."',".$item -> fields['power'].",'R',".$item -> fields['cost'].",".$item -> fields['zr'].",".$item -> fields['wt'].",".$item -> fields['minlev'].",".$item -> fields['maxwt'].",1,'N',".$intPower.",".$item -> fields['szyb'].", '".$strPtype."', ".$item -> fields['repair'].", '".$item -> fields['repair']."','".$item->fields['wzmocnienie']."',".$item->fields['ulepszeniee'].")") or error($db -> ErrorMsg());
                    }
                    else
                    {
...
if ($arrEquip[2][0])
{
    ...
    $smarty -> assign ("Helmet", $strWarn."<b>".HELMET.":</b> ".$arrEquip[2][1]." (+".$arrEquip[2][2].")(+<b>$helmspeed</b> szyb) (".$arrEquip[2][6]."/".$arrEquip[2][9]." ".DURABILITY.")".$strWarn1." [<a href=\"equip.php?schowaj=".$arrEquip[2][0]."\">".WEAR_OFF."</a>][<a href=\"equip.php?napraw=".$arrEquip[2][0]."\">".A_REPAIR."</a>]<br />\n");
}
    else
{
...
$smarty -> assign(array("Poison" => $_GET['poison'],
			"Show" => $_GET['show'],
                        "Step" => $_GET['step']));
$smarty -> display ('equip2.tpl');

//require_once(empty($_GET) || (isset($_GET['equip']) || isset($_GET['napraw']) || isset($_GET['napraw_uzywane']) || isset($_GET['sell'])) ? 'includes/foot.php' : 'includes/minifoot.php');
if (!isset($_GET['show'])) require_once("includes/foot.php");
else require_once("includes/minifoot.php");
?>
