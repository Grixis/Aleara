...
if (isset ($_GET['action']) && $_GET['action'] == 'chop')
{
    ...
    $fltAbility = abilitybonus('lumberjack') + $test -> fields['drwalnictwo'] + $drwalnictwo;
    ...
    for ($i = 0; $i < $_POST['amount']; $i++)
    {
        ...
        if ($intRoll == 5 || $intRoll == 6)
        {
            ...
            $intAmount = max(1, ceil(((rand(1, 20) * 1 / $arrKey[$intKey]) * (1 + ($fltAbility + $fltAmountAbility + $drwalnictwo + $test -> fields['drwalnictwo']) / 30)) - $arrKey[$intKey]));
            $arrAmount[$intKey] += $intAmount;
            $fltAmountAbility += 0.1;
            $intGainExp += $player -> level * 10;
        }
        if ($intRoll == 7)
        {
