...
$test = $db -> Execute("SELECT stolarstwo FROM tribeblogoslawienstwa WHERE `id`=".$player -> id);
$stolarstwo = $stolarstwo;
$smarty -> assign(array("stolarstwo" => $stolarstwo));
...
function createitem()
{
    ...
    if (rand(1,100) <= $intChance)
    {
        ...
        if (floor( rand(1,100) - $intAbility / 100) < 21)     // special item! Fletcher skill increases the chance
        {
            ...
            if ($intRoll3 < 34 && ($arrItem['type'] == 'R' or $arrItem['type'] == 'W'))  // dragon arrows, the only special item for non-artisans
            {
                $intPowerbonus = $intItembonus + $stolarstwo + $player -> strength / 50;
            ...
            if( $player -> clas == ARTISAN && $intRoll3 > 33)
            {
                ...
                if ($intRoll3 > 66)
                {
                    if ($arrItem['type'] == 'B' or $arrItem['type'] == "W")    // dwarven bow
                    {
                    ...
                    {
                        $intDurbonus = min( rand(1, ceil($player -> fletcher / 10), ceil($stolarstwo / 10)), 100);  // more arrows than usual
                    }
                    $intDur += $intDurbonus;
                    $blnSpecial = true;
                }
        ...
        $intDur = round($intDur);
        $intGainexp += $arrItem['level'] * ($blnSpecial ? 100 + $player -> fletcher + $stolarstwo / 10 : 1);
        $intItems ++;
        ...
        if ($arrItem['type'] == 'W')
        {
            ...
            $test = $db -> Execute('SELECT `id` FROM `equipment` WHERE `owner`='.$player -> id.' AND `name`=\''.$strName.'\' AND `wt`='.$intDur.'  AND `type`=\'W\' AND `status`=\'U\' AND `power`='.$intPower.' AND `zr`=0 AND `szyb`='.$intSpeed.' AND `maxwt`='.$intDur.' AND `poison`=0 AND `cost`='.$intCost);
            if (!$test -> fields['id'])
            {
                $db -> Execute('INSERT INTO `equipment` (`owner`, `name`, `power`, `type`, `cost`, `wt`, `minlev`, `maxwt`, `amount`, `magic`, `szyb`, `twohand`, `repair`) VALUES('.$player -> id.', \''.$strName.'\', '.$intPower.', \'W\', '.$intCost.', '.$intDur.', '.$arrItem['level'].', '.$intDur.', 1, \'N\',  '.$intSpeed.',\'Y\', '.$intRepaircost.')');
            }
                else
            {
        ...
        {
            $test = $db -> Execute('SELECT `id` FROM `equipment` WHERE `owner`='.$player -> id.' AND name=\''.$strName.'\' AND `power`='.$intPower.' AND `type`=\'R\' AND `status`=\'U\' AND `cost`='.$intCost);
            if (!$test -> fields['id'])
            {
...
if (isset ($_GET['mill']) && $_GET['mill'] == 'mill')
{
    ...
    */
    if (isset($_GET['konty']))
    {
        ...
        $player -> fletcher + $stolarstwo = abilitybonus('fletcher');

        /**
         * Count item attributes
         */
        if ($objLumber -> fields['type'] == 'B' or $objLumber -> fields['type'] == 'W')
        {
            $intPower = 0;
			if ($objLumber -> fields['type'] == 'W')
			{
        ...
        $intGainexp = 0;
        $intAbility = 0;
        $intChance = (50 - $intModif) * $player -> fletcher / $objLumber -> fields['level'] * $stolarstwo / $objLumber -> fields['level'];
        if ($intChance > 95)
        {
            $intChance = 95;
        }
    ...
    */
    if (isset($_GET['rob']))
    {
        ...
        foreach ($arrNeedmineral as $strNeedmineral)
        {
            ...
            {
                error(NO_MAT." Masz ".$objMineral -> fields[$strNeedmineral].", potrzeba ".$intNeedmineral);
            }
            $objMineral -> Close();
        }
        /**
