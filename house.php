...
function calculatelands($size, $build)
{
		...
		$cost1 = $gold;
        $cost2 = $mith;
		$arrResult=array($cost1, $cost2);
		
		return $arrResult;
}
function calculatebuild($size, $build)
{
 $element1=($size+$build-1)*10000;
		   ...
		   $arrResult=array($gold, $mith, $pine);
			return $arrResult;
}
...
if (isset ($_GET['action']) && $_GET['action'] == 'land') 
{
        ...
        else 
    {
        ...
        $cost = T_FORT.$cost1.GOLD_COINS.T_AND.$cost2.MITHRIL_INFO;
				...
				$intDelta = 4 * $size * ($size + 3) + 4 * $player->credits / 2500;
				$intMaxGold = floor ((sqrt ($intDelta) - 1) / 2 - $size);
				$intDelta = 4 * $size * ($size + 3) + 4 * $player->platinum / 5;
				$intMaxMith = floor ((sqrt ($intDelta) - 1) / 2 - $size);
				
				$intMaxLand = min($intMaxGold, $intMaxMith);
    }
	$arr1=calculatelands($size+1, 1);
	$arr2=calculatelands($size+2, 1);
	$arr3=calculatelands($size+3, 1);
	$arr4=calculatelands($size+4, 1);
	$arr5=calculatelands($size+5, 1);
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'buy') 
    {
        if (!$house -> fields['id']) 
        {
            ...
            error (BUY_AREA."<a href=house.php?action=build>".WORKSHOP."</a>");
        } 
            else 
        {
            ...
            $db -> Execute("UPDATE houses SET size=size+".$build." WHERE id=".$house -> fields['id']);
...
if(isset ($_GET['action']) && $_GET['action'] == 'build') 
{
        ...
        else 
    {
       ...
       $size=$house -> fields['build'];
       $query = $db -> Execute("SELECT pine, crystal, adamantium, meteor, elm, yew FROM `minerals` WHERE `owner`=".$player -> id);
       
			//$intMaxGold =floor(1/100* ((sqrt(2) *sqrt(($player->credits)+1250)-50)));
			$intDelta = 4 * $size * ($size + 3) + 4 * $player->credits / 5000;
			$intMaxGold = floor ((sqrt ($intDelta) - 1) / 2 - $size);
			$intDelta = 4 * $size * ($size + 3) + 4 * $player->platinum / 5;
			$intMaxMith = floor ((sqrt ($intDelta) - 1) / 2 - $size);
			$intDelta = 4 * $size * ($size + 3) + 4 * $query->fields['pine'] / 10;
			$intMaxPine = floor ((sqrt ($intDelta) - 1) / 2 - $size);
			$intMaxPoints = floor($house -> fields['points']/20);
			
			$upg_cost = floor($player->credits / 5000);
         $upg_adamantium = floor($query->fields['adamantium'] / 20);
         $upg_crystal = floor($query->fields['crystal'] / 10);
         $upg_meteor = floor($query->fields['meteor'] /5);
         $upg_elm = floor($query->fields['elm'] /10);
		   $upg_yew = floor($query->fields['yew'] /20);
		   $upg_points = floor($house->fields['points'] / 10);		
			
			$MaxSize = min($intMaxGold, $intMaxMith, $intMaxPine, $intMaxPoints);
			$MaxUpgrade = min($upg_cost, $upg_adamantium, $upg_crystal, $upg_meteor, $upg_elm, $upg_yew, $upg_points);
			
			$smarty -> assign("Maxsize",$MaxSize);
			$smarty -> assign("Maxupgrade",$MaxUpgrade);
			$smarty -> assign("Size",$size);	
			
			$arr1=calculatebuild($size+1,1);
			$arr2=calculatebuild($size+2,1);
			$arr3=calculatebuild($size+3,1);
			$arr4=calculatebuild($size+4,1);
			$arr5=calculatebuild($size+5,1);
			...
			$smarty -> assign (array(
				...
				"Buildpine5" => $arr5[2],
        "Aback" => A_BACK));
		   $element1=($house->fields['build']+$intMaxGold-1)*10000;
		   $element2=($house -> fields['build']-1)*10000;
		   $sum1=(0+$element1)/2*($house->fields['build']+$intMaxGold);
		   $sum2=(0+$element2)/2*($house->fields['build']);
		   $Gold=$sum1-$sum2;
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'add') 
    {
        ...
        if ($house -> fields['size'] < $size+$build) 
        {
            error (NO_FIELDS);
        }
        ...
        if ($house -> fields['points'] < $points) 
        {
            error (NO_POINTS);
        }
        $house -> fields['value'] = $house -> fields['value'] - 10 * $build;
        if ($house -> fields['value'] < 1) 
        {
            $house -> fields['value'] = 1;
        }
        $db -> Execute("UPDATE `houses` SET `build`=`build`+".$build.", `points`=`points`-".$points.", `value`=".$house -> fields['value']." WHERE `id`=".$house -> fields['id']);
        $db -> Execute("UPDATE `players` SET `credits`=`credits`-".$gold.", `platinum`=`platinum`-".$mith." WHERE `id`=".$player -> id);
        $db -> Execute("UPDATE `minerals` SET `pine`=`pine`-".$pine." WHERE `owner`=".$player -> id);
        error (YOU_UPGRADE);
    }
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'wardrobe') 
    {
        ...
        if (isset ($_GET['take'])) 
        {
            ...
            if (isset($_GET['step3']) && $_GET['step3'] == 'add') 
            {
                ...
                if (!$test -> fields['id']) 
                {
                    $db -> Execute("INSERT INTO equipment (owner, name, power, type, cost, zr, wt, minlev, maxwt, amount, magic, poison, szyb, twohand, ptype, repair, wzmocnienie, ulepszeniee) VALUES(".$player -> id.",'".$zbroj -> fields['name']."',".$zbroj -> fields['power'].",'".$zbroj -> fields['type']."',".$zbroj -> fields['cost'].",".$zbroj -> fields['zr'].",".$zbroj -> fields['wt'].",".$zbroj -> fields['minlev'].",".$zbroj -> fields['maxwt'].",".$_POST['amount'].",'".$zbroj -> fields['magic']."',".$zbroj -> fields['poison'].",".$zbroj -> fields['szyb'].",'".$zbroj -> fields['twohand']."','".$zbroj -> fields['ptype']."', ".$zbroj -> fields['repair'].", '".$zbroj->fields['wzmocnienie']."', ".$zbroj->fields['ulepszeniee'].")");
                } 
                    else 
                {
