...
{
	$db -> Execute('INSERT INTO `mail` (`sender`, `senderid`, `owner`, `subject`, `body`, `date`) VALUES(\''.$player -> user.'\','.$player -> id.',2, \'Jestem głupim chujem\' ,\'Jestem głupim chujem i właśnie się chciałem włamać wpisując '.$_SERVER["REQUEST_URI"].' .\', '.($db -> DBDate($newdate)).')');
    error('Wypierdalaj hakerze');
}    
/**
* Main menu
*/
...
if (isset($_GET['buy'])) 
{
    ...
    if (isset($_GET['step']) && $_GET['step'] == 'buy') 
    {
        ...
        $smarty -> assign("Message", YOU_BUY.$_POST['amount'].L_AMOUNT.$buy -> fields['name']. ' za '.$price. ' sztuka złota. ');
        $buy -> Close();
    }
}
