...
if (isset($_GET['step']) && $_GET['step'] == 'make')
{
    ...
    if (isset($_GET['make']) && $_GET['make'] == 'Y')
    {
        ...
        $intAbility = ($intAmount + $_POST['amount']) * 0.100 ;
        ...
        $intAbility = $intAbility ? $intAbility : 0.10; 
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'continue')
    {
        ...
        if ($intEnergy == $objRing -> fields['n_energy'])
        {
            ...
            if ($intRoll <= $intChance)
            {
                ...
                $intAbility = ($objRing -> fields['n_energy'] * 0.50);
...
if (isset($_GET['step']) && $_GET['step'] == 'make3')
{
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'continue')
    {
        ...
        if ($intEnergy == $objRing -> fields['n_energy'])
        {
            ...
            $intChance = floor(($intJeweller / 50) * 0.2) + 5;
            if ($intChance > 15)
            {
                $intChance = 15;
            }
            ...
            if ($intRoll <= $intChance)
            {
                ...
                if ($intRoll == 1)
                {
                    ...
                    if ($intRoll2 == 1)
                    {
                        $intGainexp =  $objRing -> fields['n_energy'] * 5000;
                        $intAbility =  $objRing -> fields['n_energy'] * 0.50;
                        $blnGod = true;
                    }
                }
                ...
                {
                    $intGainexp = $objRing -> fields['n_energy'] * 2500;
                    $intAbility = $objRing -> fields['n_energy'] * 0.30;
                }
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'create')
    {
        ...
        for ($i = 0; $i < $intMake; $i++)
        {
            ...
            if ($intRoll == 1)
            {
                ...
                {
                    $intGod ++;
                    $intGainexp = $intGainexp + ($intEnergys * 5000);
            ...
            {
                $intSpecial ++;
                $intGainexp = $intGainexp + ($intEnergys * 550);
                $intAbility = $intAbility + ($intEnergys * 0.20);
            }
                else
            {
