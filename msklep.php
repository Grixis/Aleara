...
if (!isset($_GET['buy'])) 
{
    $objPotions = $db -> Execute("SELECT * FROM `potions` WHERE `owner`=0 AND `status`='T' AND `lang`='".$player -> lang."' ORDER BY `power` ASC");
    $arrName = array();
    $arrPower = array();
    $arrEfect = array();
    $arrAmount = array();
    ...
    while (!$objPotions -> EOF) 
    {
        ...
        {
            $arrCost[$i] = ($objPotions -> fields['power'] * 1);
        }
            else
        {
            $arrCost[$i] = ((1 * $objPotions -> fields['power']) * 1);
        }
...
if (isset($_GET['buy'])) 
{
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'buy')
    {
        ...
        if ($objPotions -> fields['status'] != 'T') 
        {
            error(NO_TRADE);
        }
        if ($objPotions -> fields['type'] == 'M')
        {
            $intCostone = ($objPotions -> fields['power'] * 1);
        }
            else
        {
            $intCostone = ((1 * $objPotions -> fields['power']) * 1);
        }
