...
if (isset($_GET['step']) && $_GET['step'] == 'dig')
{
    ...
    if (isset($_GET['dig']) && $_GET['dig'] == 'Y')
    {
		...
		$gornictwo = $db -> GetOne("SELECT gornictwo FROM tribeblogoslawienstwa WHERE id=".$player->id);
		$fltSum = $_POST['amount'] * ($fltAbility + $gornictwo + ($_POST['amount'] - 1) * 0.05);
		$intAmount = ceil(((rand(1, 20) / $arrMinerals2[$intKey]) * ($_POST['amount'] + $fltSum / 20)) - $arrMinerals2[$intKey]); 
		//$intAmount = ceil((20/$arrMinerals2[$intKey] * ($_POST['amount'] + $fltSum / 20)) - $arrMinerals2[$intKey]); 
        if ($intAmount < 1)
        {
            $intAmount = 1;
        }
        ...
        if ($intAmount > $objMines -> fields[$_GET['mine']])
        {
			...
			//$_POST['amount']=( $intAmount * $arrMinerals2[$intKey] + pow($arrMinerals2[$intKey],2) - $fltSum)/20;
