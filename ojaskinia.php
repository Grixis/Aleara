...
*/
require_once("languages/".$player -> lang."/ojaskinia.php");
...
if (isset($_GET['step']) && $_GET['step'] == 'run') 
{
    ...
    if ($hp -> fields['hp'] > 0) 
    {
        $smarty -> assign("Link", "<a href=\"ojaskinia.php?action=explore\">".A_EXPLORE."</a>");
    }
    $hp -> Close();
}
...
if (isset($_GET['action']) && $_GET['action'] == 'explore')
{
    ...
    $intRoll = rand(1, 100);
    $smarty -> assign(array("Link" => "<a href=\"ojaskinia.php?action=explore\">".A_EXPLORE."</a>",
        "Roll" => $intRoll));
    if ($intRoll < 49)
    {
