...
window.addEvent("load",function(){if(window.ie6){$$("#<?php echo $menu_ID;?> li").each(function(e){e.addEvents({"mouseenter":function(){e.addClass("sfHover");},"mouseleave":function(){e.removeClass("sfHover");}});});}});

<?php if($_GET['style'] == 'moomenu') : ?>
window.addEvent("domready", function(){
	...
	main.getChildren().each(function(el,i){
		...
		el.getElementsBySelector("ul").each(function(elm,j){
			...
			<?php if($_GET['opacity'] == 1) : ?>opacityFX[i].push(new Fx.Opacity(elm, {duration: <?php echo $_GET['speed'] ?>, transition: <?php echo $animation; ?>}).set(0));<?php endif; ?>
			<?php if($_GET['height'] == 1) : ?>heightFX[i].push(new Fx.Height(elm, {duration: <?php echo $_GET['speed'] ?>, transition: <?php echo $animation; ?>}).set(0));<?php endif; ?>
			<?php if($_GET['width'] == 1) : ?>widthFX[i].push(new Fx.Width(elm, {duration: <?php echo $_GET['speed'] ?>, transition: <?php echo $animation; ?>}).set(0));<?php endif; ?>
		});
	});
