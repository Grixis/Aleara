...
*/
require_once("languages/".$player -> lang."/salerozpaczy2.php");
...
$objRings = $db -> Execute("SELECT `id`, `name`, `amount` FROM `salerozpaczy2`");
$i = 0;
$arrId = array();
$arrName = array();
$arrAmount = array();
...
if (isset($_GET['buy']))
{
    ...
    $db -> Execute("UPDATE `salerozpaczy2` SET `amount`=`amount`-1 WHERE `id`=".$_GET['buy']);
    $objTest = $db -> Execute("SELECT `id` FROM `equipment` WHERE `owner`=".$player -> id." AND `name`='".$arrName[$intKey]."' AND `status`='U' AND `cost`=50");
    if (!$objTest -> fields['id'])
    {
        $db -> Execute("INSERT INTO `equipment` (`owner`, `name`, `power`, `status`, `type`, `cost`, `minlev`, `amount`) VALUES(".$player -> id.", '".$arrName[$intKey]."', '200', 'U', 'I', '50', '30', '1')");
    }
        else
    {
    ...
    $strMessage = YOU_BUY.$arrName[$intKey].FOR_A." <a href=\"salerozpaczy2.php\">".A_REFRESH."</a>";
}
...
$smarty -> assign(array("Shopinfo" => SHOP_INFO,
                        ...
                        "Abuy" => A_BUY,
                        "Message" => $strMessage));
$smarty -> display ('salerozpaczy2.tpl');
