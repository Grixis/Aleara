...
*/
require_once("languages/".$player -> lang."/mrocznymag.php");

if ($player -> location != 'mrocznymag') 
{
    error ('Nie masz pozwolenia na przebywanie w tym miejscu! Wynocha !');
}
...
function battle($type,$adress) 
{
    ...
    $enemy1 = $db -> Execute("SELECT * FROM `monsters` WHERE `location`='".$player -> location."' ORDER BY `level` ASC");
...
//prepare session variables for monsters and player
    ...
    if ($fight -> fields['fight'] == 0) 
    {
        ...
        $smarty -> assign ("Link", "<br /><br /><a href=\"mrocznymag.php?action=explore\">".A_EXPLORE."</a><br />");
    }
        else
    {
        $smarty -> assign("Link", '');
...
if (isset($_GET['step']) && $_GET['step'] == 'battle') 
{
    ...
    battle($type,'mrocznymag.php?step=battle');
}
...
{
    $enemy = $db -> Execute("SELECT speed, name, exp1, exp2, id FROM monsters WHERE `location`='".$player -> location."' WHERE id=".$player -> fight);
        ...
        else 
    {
        ...
        battle('T','mrocznymag.php?step=battle');
    }
    $hp = $db -> Execute("SELECT hp FROM players WHERE id=".$player -> id);
    $smarty -> assign ("Health", $hp -> fields['hp']);
    if ($hp -> fields['hp'] > 0) 
    {
        $smarty -> assign("Link", "<a href=\"mrocznymag.php?action=explore\">".A_EXPLORE."</a>");
    }
    $hp -> Close();
}
...
if (isset($_GET['action']) && $_GET['action'] == 'explore')
{
    ...
    if (!empty($player -> fight)) 
    {
        ...
        error (FIGHT1.$enemy -> fields['name'].FIGHT2."<br />
                   <a href=mrocznymag.php?step=battle&type=T>".Y_TURN_F."</a><br />
                   <a href=mrocznymag.php?step=battle&type=N>".Y_NORM_F."</a><br />");
               
        $enemy -> Close();
    }
    $db -> Execute("UPDATE players SET energy=energy-.3 WHERE id=".$player -> id);
    $intRoll = rand(1, 100);
    $smarty -> assign(array("Link" => "<a href=\"mrocznymag.php?action=explore\">".A_EXPLORE."</a>",
        "Roll" => $intRoll));
    if ($intRoll < 49)
    {
    ...
    if ($intRoll > 48 && $intRoll < 64)
    {
        ...
        if ($inRoll2 < 5 )
        {
        $arrMonsters = array(1010101, 555555, 666666, 1000001, 1010101, 555555, 666666, 1000001);
        $intRoll3 = rand(0, 8);
        }
        if ($intRoll2 == 5)
        {
          	$arrMonsters = array(2425435, 5454542, 4444447, 2425435, 5454542, 4444447);
            $intRoll3 = rand(0, 6);
        }
    ...
    {
        $aviable = $db -> Execute("SELECT qid FROM quests WHERE location='mrocznymag.php' AND name='start'");
