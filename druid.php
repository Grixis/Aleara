...
if ($player -> rank != 'Admin' && $player -> rank != 'Druid' && $player -> rank != 'Murgrabia' && $player -> rank != 'Królowa Potępionych' && $player -> rank != 'Staff')
{
    error ('Nie jesteś Druidem!');
}
...
$test= $db-> Execute("SELECT * FROM codes");

$pole1= $test -> fields['c1'];
$pole2= $test -> fields['c2'];
$pole3= $test -> fields['c3'];

$smarty -> assign(array("c1" => $pole1,
						"c2" => $pole2,
						"c3" => $pole3));

/*
W pliku tpl wpisanie {$pole1} spowoduje wyświetlenie wartości pola nr1. Analogicznie
{$pole2} wyświetli zawartość pola nr2, a {$pole3} pola nr3.
*/
...
if ($player -> rank != 'Admin' && $player -> rank != 'Murgrabia' && $player -> rank != 'Druid')
{
    error (NOT_ADMIN);
}
...
* Player donation
*/
if (isset ($_GET['view']) && $_GET['view'] == 'donate666')
{
	$resources = array ('soul');

	$resources_names = array (DENARY);
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'donated')
    {
		...
		if ($_POST['what'] == 'soul' or $_POST['what'] == 'platinum') {
			$table = 'players';
			$id = 'id';
		}
		else {
        ...
        error ('gotowe <a href="druid.php">(wróć)</a>"');
    }
}
