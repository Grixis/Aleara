 ...
 */
	function backpack ($playerid)
	{
		...
		if ($this -> step == 'wzmocnij') {
			...
			$arm = $db -> Execute('SELECT * FROM `equipment` WHERE `owner`='.$playerid.' AND `type`!=\'C\' AND `type`!=\'T\' AND name NOT LIKE "%Kostur%" AND `status`!=\'E\'');
		}
 ...
 */
	function przetop_wszystkie () {
		...
		$smelter_bonus=$test->fields['hutnictwo'];
		
		if (!$smelter_bonus) { $smelter_bonus=0; }
		if (!empty($this->action) && $this->action!='') {
			...
			while (!$arm -> EOF)     //PHP4 ssie while=>foreach
			{
				...
				for ($i=0;$i<$arm->fields['amount'];$i++) {
					$szansaPrzetopienia=50+($player->hutnictwo+$smelter_bonus)-$arm->fields['minlev']*8*$arrWspSurowca[$intKeySztabka];
				...
				$intOdzyskaneSztabki=ceil(($player->hutnictwo+$smelter_bonus)/$arrWspSurowca[$intKey]*10);
	...
	function przetop () {
		...
		if (!empty($this->action) && $this->action>0) {
			...
			*/
			...
			for ($i=0;$i<$amount;$i++) {
				$szansaPrzetopienia=50+($player->hutnictwo+$smelter_bonus)-$arm->fields['minlev']*8*$arrWspSurowca[$intKey];
				if ($szansaPrzetopienia>95) {
					$szansaPrzetopienia=95;
				}
				$rand=rand(1,100);
	...
	function wzmocnij () {
		...
		if (!empty($this->action) && $this->action>0) {
			...
			if (isset($_POST['typ'])) {
				...
				//zmieniony na:
				$szansa=rand(($player->hutnictwo+$smelter_bonus)/(50*$iModifier),$player->hutnictwo/(5*$arrModyfikator[$intKey]*$iModifier));
				...
				if ($szansa>$szansaTest) {
					...
					$silaWzmocnienia = ceil(($player -> hutnictwo+$smelter_bonus) / (20 * $iModifier));
 ...
 */
	function wytop () {
		...
		foreach ($arrSmeltAmount AS $array) {  //ile mozna maksymalnie przeznaczyc energii na wytop
			unset($ileMax);
			$i=0;
			$energiiNaSztabke[$j] = round(1 / (10 * ( 0.25 / $arrWspI[$j] + ($player -> hutnictwo+$smelter_bonus) / (50 + 30 * $arrWspI[$j]) ) ) , 2);
		...
		if (isset($_GET['action']) && $this->action!='Y') //Wybrano jaka sztabka
		{
			...
			if (isset($_POST['amount'])) // Ile energii
			{
				...
				$energiiNaSztabke = round(1 / (10 * ( 0.25 / $arrWspI[$intKey] + ($player -> hutnictwo+$smelter_bonus) / (50 + 30 * $arrWspI[$intKey]) ) ), 2);
				...
				for ($i=0;$i<$triesAmount;$i++) {
					$szansaWytopienia=floor(100+$hutnictwo+$test -> fields['hutnictwo']+($player->hutnictwo+$smelter_bonus)-8*$arrWspSurowca[$intKey]);
