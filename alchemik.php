...
$smarty -> assign(array("alchemia" => $alchemia));
        ...
        for ($i = 1; $i <= $_POST['razy']; $i++)
        {
            ...
            $intChance = (($player -> level - $objItem2 -> fields['level'])* 5) + ($alchemia / 3) + ($player -> alchemy / 3) + $fltStat;
            if ( $intChance < 0)
            {
                $intChance = 0;
            }
            ...
            if ($intTmpamount)
            {
                ...
                if ($player -> clas == 'Rzemieślnik' && $intRoll2 > 89 && $objItem -> fields['type'] != 'A')
                {
                    ...
                    $strName = $kuznia -> fields['name']." (S)";
                     $rpd = ($rpd + $kuznia -> fields['level'] * 4);
                    if ($intTmpamount > 1)
                    {
                        $rpd = ($rpd + (($kuznia -> fields['level'] / 150) * (80 * ($intTmpamount - 1))));
                    }
                }
                    else
                {
            ...
            $test = $db -> Execute('SELECT `id` FROM `potions` WHERE `name`=\''.$strName.'\' AND `owner`='.$player -> id.' AND `status`=\'K\' AND `power`='.$intPower) or die($db->ErrorMsg());
            if (!$test -> fields['id'])
            {
