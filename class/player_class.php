...
// $Id: player_class.php 905 2007-02-28 21:33:05Z thindil $
    ...
    function logIn($strEmail, $strPass, $pllimit, $finger) ///TODO: make it public static on php5 server
    {
        global $db;
        global $smarty;
        if (!$strEmail || !$strPass)
        {
            $smarty -> assign ('Error', "Wypełnij wszystkie pola.");
            $smarty -> display ('error.tpl');
            exit;
        }
        // Attempt to get player data.
        $strEmail = $db -> qstr($strEmail, get_magic_quotes_gpc());
        $query = $db -> Execute('SELECT `id`, `user`, `email`, `rank`, `freeze`, `fight`, `fingerprint` FROM `players` WHERE `email`='.$strEmail.' AND `pass`=\''.$strPass.'\'') or die($db -> ErrorMsg());
        ...
        if (!$query -> fields['id'])
        {
            $smarty -> assign ('Error', "Nie ma takiego gracza");
            $smarty -> display ('error.tpl');
            exit;
        }
        if ($query -> fields['freeze'])
        {
            $smarty -> assign('Error', "Wybrałeś opcję zamrożenia konta. Konto będzie aktywne za ".$query -> fields['freeze']." dni");
            $smarty -> display ('error.tpl');
            exit;
        }
        // Check for bans.
        $objBan = $db -> Execute('SELECT `type` FROM `ban` WHERE `type`=\'IP\' AND `amount`=\''.$_SERVER['REMOTE_ADDR'].'\' OR `type`=\'ID\' AND `amount`='.$query -> fields['id'].' OR `type`=\'nick\' AND `amount`=\''.$query -> fields['user'].'\' OR `type`=\'mailadres\' AND `amount`='.$strEmail.' OR `type`=\'BR\' AND `amount`="'.$finger.'"');
        if ($objBan -> fields['type'])
        {
            setcookie("intkey", 1);
			$objBan -> Close();
            $smarty -> assign ('Error', "Za niecne występki zostałeś skazany na banicję z krainy. Strażnicy nie wpuszczą cię.");
			
            $smarty -> display ('error.tpl');
            exit;
        }
        //$objBan2= $db->Execute('Select type from ban where type="BR")
        ...
        $strSql = 'UPDATE `players` SET `logins`=`logins`+1, `rest`=\'N\', `fingerprint`="'.$finger.'"';
...
*/
    ...
    {   /// TODO: zmienić nazwy pól w bazie i w klasie
        ...
        $stats = $db -> Execute('SELECT `id`, `user`, `lang`, `rank`, `level`, `exp`, `energy`, `max_energy`, `hp`, `max_hp`, `pm`, `credits`, `bank`, `platinum`, `tribe`, `tribe_rank`, `miejsce`, `style`, `graphic`, `graphbar`, `graphstyle`, `inteli`, `wisdom`, `fight`, `rasa`, `klasa`, `deity`, `gender`, `overlib`, `mailinfo`, `bles`, `soul`, `chaos`, `pokrzywa`, `kora`, `wlos`, `loginfo` FROM `players` WHERE `email`=\''.$strEmail.'\'');
        ...
        $this -> buu = $stats -> fields['buu'];
        
        $this -> soul = $stats -> fields['soul'];
        $this -> bles = $stats -> fields['bles'];
        $this -> chaos = $stats -> fields['chaos'];
        $this -> pokrzywa = $stats -> fields['pokrzywa'];
        $this -> kora = $stats -> fields['kora'];
        $this -> wlos = $stats -> fields['wlos'];
    ...
    {//race class, 6 cech (minus int i sw), ability, fletcher, alchemia, jeweller, platinum
        global $db;
        $stats = $db -> Execute('SELECT `rasa`, `klasa`, `gender`, `platinum`, `strength`, `agility`, `szyb`, `wytrz`, `ability`, `fletcher`, `alchemia`, `jeweller`, `hutnictwo`, `bles`, `soul`, `chaos`, `pokrzywa`, `kora`, `wlos` FROM `players` WHERE `id`='.$this -> id);
        $this -> race = $stats -> fields['rasa'];
        $this -> clas = $stats -> fields['klasa'];
        $this -> gender = $stats -> fields['gender'];
        $this -> platinum = $stats -> fields['platinum'];
        ...
        $this -> gotowanie = $stats -> fields['gotowanie'];
        $this -> ziemniaki = $stats -> fields['ziemniaki'];
        $this -> gotow = $stats -> fields['gotow'];
        $this -> ryby = $stats -> fields['ryby'];
        $this -> grzyby = $stats -> fields['grzyby'];
        $this -> soul = $stats -> fields['soul'];
        $this -> bles = $stats -> fields['bles'];
        $this -> chaos = $stats -> fields['chaos'];
           $this -> pokrzywa = $stats -> fields['pokrzywa'];
...
/*    function description($intId)
    {   global $db;
/// TODO: Dorobić żeby pokazywał cudzy profil. Immun. Profil wyjebać
