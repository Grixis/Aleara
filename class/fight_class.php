	...
	//czry maga
		...
		$this -> setDamage();
	        ...
	        $this -> setAttackSpeed($arrEquip);
                ...
                $this -> debug= "Spd:".$this-> speed."| W:".$arrEquip[0][7]."| B:".$arrEquip[1][7]."| H:".$arrEquip[2][7]."| A:".$arrEquip[3][7]."| L:".$arrEquip[4][7]."| S:".$arrEquip[5][7]."| Def:".$this->defence."| Dmg:".$this->damage;
...
*/
	...
	if (!isset($enemy -> id)) {
		...
		if ($enemy -> hp > 0 && $this -> hp > 0) {
			...
			$db -> Execute("UPDATE tribes SET prest=prest+2 WHERE id=".$this -> tribe);
			$db -> Execute("UPDATE `gielda` SET `cost`=`cost`+5 WHERE `id`=".$this -> tribe);
			}
		else {
			$stats = array('strength','agility','inteli','wytrz','szyb','wisdom');
...
*/
	...
	$test=$db->Execute("Select * from tribeblogoslawienstwa where id=".$this->id);
	if ($test -> fields['id'])
		{
			$this -> strength += $test-> fields['sila'];
			$this -> agility += $test-> fields['zrecznosc'];
			$this -> speed += $test-> fields['szybkosc'];
			$this -> inteli += $test-> fields['inteligencja'];
			$this -> wisdom += $test-> fields['silawoli'];
			$this -> cond += $test-> fields['wytrzymalosc'];
		}
	$test2=$db->Execute("Select SUM(szyb) from equipment where owner=".$this->id ." AND status='E' AND type IN ('W','B','A', 'H', 'L', 'S')");
	if ($test2-> fields['SUM(szyb)'])
	{
		$this ->speed += $test2->fields['SUM(szyb)'];
	}
    $test2=$db->Execute("Select SUM(zr) from equipment where owner=".$this->id ." AND status='E' AND type IN ('W', 'B', 'H', 'A', 'L', 'S')");
	if ($test2-> fields['SUM(zr)'])
	{
		$this ->agility += -$test2->fields['SUM(zr)'];
	}
...
*/
	...
	$test2=$db->Execute("Select SUM(szyb) from equipment where owner=".$this->id ." AND status='E' AND type IN ('A', 'H', 'L', 'S')");
	if ($test2-> fields['SUM(szyb)'])
	{
		//$this ->speed -= $test2->fields['SUM(szyb)'];
	}
...
*/
	...
	if ($arrEquip[10][2]) {
		...
		$intAmount = count($arrRingtype) - 1;
		$intKey = array_search($arrRingtype[$intAmount], $arrRings);
		$this -> $arrStat[$intKey] += $arrEquip[10][2];
		}
	}
...
//bonusy klasowe
		if ($this -> clas == 'Wojownik' || $this -> clas == 'Barbarzyńca') {
			...
			$this -> damage -= ($this -> damage)/4;
				...
				else if ($this -> clas == 'Obywatel') {
			$this -> damage -= ($this -> damage)/2;
			}
				else if ($this -> clas == 'Artysta') {
...
*/
	...
	if ($this -> equipment[1][6]) {
		if($this -> equipment[6][6]) {
			...
			$this -> damage = $this -> equipment[1][2] + $this -> equipment[6][2] + $this -> agility / 1.5 + $this -> strength / 2;
			//$this -> mindamage = $this -> agility / 2 + $this -> strength / 2;
			//$this -> maxdamage = $this -> strength / 2 + $this -> agility /2 + $damage;
//bonusy klasowe
			if ($this -> clas == 'Wojownik' || $this -> clas == 'Barbarzyńca') {
				$this -> damage += $this -> level;
				}
...
//premia rozdzki
		if ($this -> equipment[7][0]) {
			...
			$multi=10*$this->level/$this->equipment[7][4];
			$this -> damage += rand($multi,$multi*$this->equipment[7][4]);
			}
//kary z pancerzy
		for ($i=2;$i<=5;$i++) {
...
//czar maga
	...
	//premia rozdzki
		if ($arrEquip[7][0]) {
			...
			$multi=10*$this->level/$this->equipment[7][2];
			$this -> defence += $this->equipment[7][2];
			}
	//kary z pancerzy
		for ($i=2;$i<=5;$i++) {
			if ($arrEquip[$i][6]) {
...
//czar
	...
	$this -> criticalhit =($this -> criticalstat) / 300;
	if ($this -> criticalhit > 30) {
		$this -> criticalhit = 30;
		}
...
*/
    ...
    /**
     * Funkcja zwraca ekwipunek jaki mamy na sobie.
     
     * $arrEquip[typ][wartość]
...
7 - różdżka
8 - szata
9 - pierścień slot pierwszy
10 - pierścień slot drugi
...
4 - minimalny poziom
5 - premia zręczności
6 - wytrzymałość ( w przypadku strzał to ich ilość)
7 - premia do szybkości
8 - siła trucizny
9 - typ trucizny ( porównywany jest z wypitym typem antidotum )
10 - maksymalna wytrzymałość przedmiotu.
     */
    function equipment()
    {
        global $db;
