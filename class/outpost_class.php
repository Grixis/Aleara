...
*/
	...
	function addVeteran(&$arrVeterans)
	{
		...
		$arrValues = array($_POST['W'],$_POST['B'],$_POST['A'],$_POST['S'],$_POST['H'],$_POST['L'],$_POST['I1'],$_POST['I2']);
		...
		$query="Insert into outpost_veterans (name, outpost, weapon, wpower, bow, bpower, armor, apower, shield, spower, helm, hpower, legs, lpower, ring1, r1power, ring2, r2power) values ('".$_POST['name']."',".$this->id.", ";
		for ($i=0;$i<=count($arrKeys2)-1;$i++)
		{
         $newKey = $arrKeys2[$i];
         $newKey = $newKey{0};
         ...
         if ($arrValues[$i] != 0)
         {
             $arrTest =  $this -> db -> GetRow('SELECT `name`, '.($arrKeys2[$i] == 'B' ? '`szyb`' : '`power`').', `amount` FROM `equipment` WHERE `id`='.$arrValues[$i] .' AND `owner`='. $this -> ownerId. ' AND `type`=\''. $newKey. '\' AND `status`=\'U\'');
             $query .="'".$arrTest[0]."',".$arrTest[1];
             if ($i != count($arrKeys2)-1)
             {
             $query .=",";
             }
             if ($arrTest[2] == 1)
					     $this -> db -> Execute('DELETE FROM `equipment` WHERE `id`='.$arrValues[$i]);
				     else
					     $this -> db -> Execute('UPDATE `equipment` SET `amount`=`amount`-1 WHERE `id`='.$arrValues[$i]);
         }
         else
         {
             if ($i == count($arrKeys2)-1)
	...
	function modifyVeteran(&$arrVeterans)
	{
		...
		$query = "UPDATE `outpost_veterans` SET `name`='".$_POST['name']."', ";
    ...
    for ($i=0;$i<=count($arrKeys2)-1;$i++)
		{
         ...
         if ($arrValues[$i] != 0 && $arrValues[$i] != 2147483647)
         {
             $arrTest =  $this -> db -> GetRow('SELECT `name`, '.($arrKeys2[$i] == 'B' ? '`szyb`' : '`power`').', `amount` FROM `equipment` WHERE `id`='.$arrValues[$i] .' AND `owner`='. $this -> ownerId. ' AND `type`=\''. $newKey. '\' AND `status`=\'U\'');
             $query .=$arrSQLItemNames[$i]."='".$arrTest[0]."',".$arrSQLItemPowers[$i]."=".$arrTest[1];
             if ($i != count($arrKeys2)-1)
             {
             $query .=", ";
             }
         ...
         elseif($arrValues[$i] == 0)
         {
             if ($i == count($arrKeys2)-1)
             {
             $query .=$arrSQLItemNames[$i]."='', ".$arrSQLItemPowers[$i]."=0";
             }
             else
             {
             $query .=$arrSQLItemNames[$i]."='', ".$arrSQLItemPowers[$i]."=0, ";
