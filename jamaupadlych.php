...
$title = 'Jama Upadłych w Diabolium';
require_once('includes/head.php');
require_once('languages/'.$player -> lang.'/jamaupadlych.php');
    ...
    $smarty -> assign(array('Daddday' => D_ADDDAY,
        ...
        if (isset ($_GET['step']) && $_GET['step'] == 'payday')
        {
                ...
                if ($podklasa != "Handlarz")
	              {
		            error("Nie jesteś handlarzem!");
	              }
                $db -> Execute("INSERT INTO `vault` (`owner`, `time`, `amount`, `type`) VALUES (".$player -> id.", '168',".$_POST['amount'].", '50')");
...
if (isset ($_GET['akcja']) && $_GET['akcja'] == 'handlarz')

{
    $smarty -> assign(array('Daddday' => D_ADDDAY,
    'Dhandlarz' => D_HANDLARZ,
...
if (isset ($_GET['akcja']) && $_GET['akcja'] == 'addweek')
{
    $smarty -> assign(array('Waddweek' => W_ADDWEEK,
        ...
        if (isset ($_GET['step']) && $_GET['step'] == 'payweek')
        {
                ...
                $db -> Execute("INSERT INTO `vault` (`owner`, `time`, `amount`, `type`) VALUES(".$player -> id.", '100',".$_POST['amount'].", '20')");
                $db -> Execute("UPDATE `players` SET `credits`=`credits`-".$_POST['amount']." WHERE `id`=".$player -> id);
...
$smarty -> display ('jamaupadlych.tpl');
