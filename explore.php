...
if (isset($_GET['action']) && $_GET['action'] == 'moutains' && $player -> location == 'Góry' && !isset($_GET['step'])) 
{
    ...
    for ($i = 1; $i < $intAmount2; $i++)
    {
        ...
        if ($intRoll == 13) 
        {
            ...
            if ($intRoll2 < 5 )
            {
            	$arrMonsters = array(2000, 4000, 12000, 5000, 17000, 2000, 2000, 17000, 2000);
                $intRoll3 = rand(0, 8);
            }
            if ($intRoll2 == 5)
            {
            	$arrMonsters = array(2000, 10000, 2000, 10000);
                $intRoll3 = rand(0, 3);
            }
    ...
    if ($intHerbsum || $intGoldsum || $intAstral || $intMeteor || $drzewka)
    {
...
if (isset($_GET['action']) && $_GET['action'] == 'forest' && $player -> location == 'Las')
{
    ...
    for ($i = 1; $i < $intAmount; $i++)
    {
        ...
        if ($intRoll == 13) 
        {
            ...
            if ($intRoll2 < 5 )
            {
            	...
            	$arrMonsters = array(2000, 4000, 12000, 5000, 2000, 17000, 2000, 2000, 17000);
                $intRoll3 = rand(0, 7);
            }
           
			if ($intRoll2 == 5)
            {
            	$arrMonsters = array(2000, 10000, 17000, 10000);
                $intRoll3 = rand(0, 3);
            }
    ...
    $fltAmount = $i / 2;
    $strFind = YOU_GO.$fltAmount.T_AMOUNT2;
    if ($intHerbsum || $intGoldsum || $intAstral || $drzewka)
    {
        $strFind = $strFind.YOU_FIND;
        $arrAmounts = array($arrHerbs[0], $arrHerbs[1], $arrHerbs[2], $arrHerbs[3], $intAstral, $arrGold[0], $arrGold[1], $arrGold[2], $drzewka);
        $arrText = array(HERB1, HERB2, HERB3, HERB4, T_ASTRALS, T_GOLD, T_ENERGY2, T_MAPS, " sadzonek drzewek");
        $i = 0;
        foreach ($arrAmounts as $intAmount)
        {
            if ($intAmount)
    ...
    if (!$intHerbsum && !$intGoldsum && !$intAstral && $strEnemy == '' && !$drzewka)
    {
        $strFind = $strFind.FIND_NOTHING;
    }
    $fltEnergy = $fltAmount - $arrGold[1];
    $db -> Execute("UPDATE `players` SET `credits`=`credits`+".$arrGold[0].", `energy`=`energy`-".$fltEnergy.", `drzewka`=`drzewka`+".$drzewka.", `maps`=".$player -> maps." WHERE `id`=".$player -> id);
    $db -> Execute("UPDATE `settings` SET `value`=".$objMaps -> fields['value']." WHERE `setting`='maps'");
    $objMaps -> Close();
    $smarty -> assign(array("Youfind" => $strFind,
                            "Howmuch" => HOW_MUCH,
