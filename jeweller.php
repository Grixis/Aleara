...
*/
require_once('languages/pl/jeweller.php');
...
if (isset($_GET['step']) && $_GET['step'] == 'make')
{
    ...
    if (isset($_GET['make']) && $_GET['make'] == 'Y')
    {
        ...
        $intAbility = ($intAmount + $_POST['amount']) * 0.0 ;
        $intGainexp = $intAmount;
        if ($player -> clas == ARTISAN)
        {
            $intAbility = $intAbility * 1;
            $intGainexp = $intGainexp * 1.9;
        }
        $intAbility = round($intAbility*1, 0)/100;
        ...
        if ($intAmount)
        {
            $objTest = $db -> Execute('SELECT `id` FROM `equipment` WHERE `owner`='.$player -> id.' AND type=\'I\' AND power=0 AND `status`=\'U\'');
...
if (isset($_GET['step']) && $_GET['step'] == 'make2')
{
    ...
    if (!isset($_GET['action']))
    {
        ...
        if (!$objMaked -> fields['id'])
        {
            ...
            while (!$objPlans -> EOF)
            {
                ...
                $arrCryst[$i] = ceil($objPlans -> fields['level'] / 1);
                $arrMeteor[$i] = ceil($objPlans -> fields['level'] / 1);
                $arrBonus[$i] = $objPlans -> fields['bonus'] * 3;
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'continue')
    {
        ...
        if ($intEnergy == $objRing -> fields['n_energy'])
        {
            ...
            if ($intRoll <= $intChance)
            {
                ...
                $objTest -> Close();
                $intGainexp = $objRing2 -> fields['level'] * 2000;
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'create')
    {
        ...
        $intCryst = ceil($objRing -> fields['level'] / 1) * $intNeed;
        $intMeteor = ceil($objRing -> fields['level'] / 1) * $intNeed;
        ...
        $objRings = $db -> Execute('SELECT `id`, `amount` FROM `equipment` WHERE `owner`='.$player -> id.' AND type=\'I\' AND power=0 AND `status`=\'U\'');
		
        if ($objRings -> fields['amount'] < $intMake)
        {
            error(NO_RINGS.' Masz tylko '.$objRings->fields['amount']);
        }
        ...
        for ($i = 0; $i < $intMake; $i++)
        {
            ...
            if ($intRoll <= $intChance)
            {
                ...
                $intBonus = rand(floor($intJeweller/2), $intJeweller);
                if ($intBonus < 1)
                {
                    $intBonus = 1;
                }
        ...
        if ($intMake > 0)
        {
            ...
            $intGainexp = $objRing -> fields['level'] * 250;
            $intGainexp = floor($intGainexp * $intAmount2);
            $intAbility = 0.01 * $objRing -> fields['level'] * ($intAmount2 + $intMake);
            ...
            else
        {
            ...
            $intCryst = ceil($objRing -> fields['level'] / 1);
            $intMeteor = ceil($objRing -> fields['level'] / 1);
            $intGainexp = 0;
...
if (isset($_GET['step']) && $_GET['step'] == 'make3')
{
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'continue')
    {
        ...
        if ($intEnergy == $objRing -> fields['n_energy'])
        {
            ...
            $intChance = floor(($intJeweller / 35) * 0.2) + 5;
            if ($intChance > 15)
            {
                $intChance = 15;
            }
            ...
            if ($intRoll <= $intChance)
            {
                ...
                if ($intRoll == 1)
                {
                    ...
                    if ($intRoll2 == 1)
                    {
                        $intGainexp =  $objRing -> fields['n_energy'] * 100000;
                        $intAbility =  $objRing -> fields['n_energy'] * 0.10;
                        $blnGod = true;
                    }
                }
                if ($intRoll <= $intChance && !$blnGod)
                {
                    $intGainexp = $objRing -> fields['n_energy'] * 200000;
                    $intAbility = $objRing -> fields['n_energy'] * 0.10;
                }
            ...
            {
                $intAbility = $objRing -> fields['n_energy'] * 0.20;
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'create')
    {
        ...
        $intMeteor = ceil($intLevel / 1) * $intMake;
        ...
        for ($i = 0; $i < $intMake; $i++)
        {
            ...
            if ($intRoll == 1)
            {
                ...
                if ($intRoll2 == 1)
                {
                    ...
                    $intAbility = $intAbility + ($intEnergys * 0.20);
                    continue;
                }
            }
            if ($intRoll <= $intChance)
            {
                $intSpecial ++;
                $intGainexp = $intGainexp + ($intEnergys * 400);
                $intAbility = $intAbility + ($intEnergys * 0.20);
            }
                else
            {
                $intAbility = $intAbility + 0.20 * $intEnergys;
            }
        }
